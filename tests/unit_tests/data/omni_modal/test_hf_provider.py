...
"""Unit tests for HFOmniModalDatasetProvider."""
...
def _make_provider() -> HFOmniModalDatasetProvider:
    return HFOmniModalDatasetProvider(
        seq_length=32,
...
def test_build_datasets_happy_path(monkeypatch):
    ...
    monkeypatch.setattr("megatron.bridge.data.omni_modal.hf_provider.is_safe_repo", fake_is_safe_repo)
    monkeypatch.setattr("megatron.bridge.data.omni_modal.hf_provider.load_dataset", fake_load_dataset)
    monkeypatch.setattr("megatron.bridge.data.omni_modal.hf_provider.AutoProcessor", _AutoProcessor)
    monkeypatch.setattr("megatron.bridge.data.omni_modal.hf_provider.AutoTokenizer", _AutoTokenizer)
