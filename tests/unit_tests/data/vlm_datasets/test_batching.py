...
def test_pad_or_pack_sequence_batch_pads_to_efficiency_multiple():
    batch = {
...
def test_pad_or_pack_sequence_batch_pads_to_model_length_when_required():
    batch = {
        "input_ids": torch.tensor([[1, 2, 3]]),
        "labels": torch.tensor([[2, 3, -100]]),
        "loss_mask": torch.tensor([[1.0, 1.0, 0.0]]),
    ...
    pad_or_pack_sequence_batch(batch, sequence_length=6, pad_to_max_length=True)
...
def test_pad_or_pack_sequence_batch_handles_rectangular_4d_attention_mask():
    batch = {
        "input_ids": torch.tensor([[1, 2, 3]]),
        "labels": torch.tensor([[2, 3, -100]]),
        "loss_mask": torch.tensor([[1.0, 1.0, 0.0]]),
    ...
    pad_or_pack_sequence_batch(batch, sequence_length=4, pad_to_max_length=True)
...
def test_pad_or_pack_sequence_batch_packs_and_emits_metadata():
    batch = {
...
def test_pad_or_pack_sequence_batch_packs_with_legacy_unpadded_aliases_without_extra_padding():
    batch = {
        "input_ids": torch.tensor(
            [
                [1, 2, 3, 0, 0],
