...
class TestDataGPTSFTDataset:
    ...
    def test_batch_dataloader_packs_each_logical_microbatch(self, tmp_path):
        dataset, _ = get_gpt_sft(
            tmp_path,
            max_num_samples=None,
            enable_in_batch_packing=True,
        ...
        global_batch = next(iter(dataloader))
        microbatch_iter, seq_length = prepare_finetuning_batch(
            iter([global_batch]),
            num_microbatches=global_batch_size // micro_batch_size,
            default_seq_length=dataset.max_seq_length,
        )
        microbatches = list(microbatch_iter)

        assert len(microbatches) == 2
        assert seq_length == max(microbatch["tokens"].size(1) for microbatch in microbatches)
        assert all(microbatch["tokens"].size(0) == 1 for microbatch in microbatches)
        assert all(len(microbatch["metadata"]) == micro_batch_size for microbatch in microbatches)
        assert all(microbatch["cu_seqlens_q"].numel() == micro_batch_size + 1 for microbatch in microbatches)
        assert sum(len(microbatch["metadata"]) for microbatch in microbatches) == global_batch_size

    def test_batch_dataloader_packed_final_batch_keeps_padding_rows_at_zero_loss(self, tmp_path):
        dataset, _ = get_gpt_sft(
            tmp_path,
            max_num_samples=None,
            enable_in_batch_packing=True,
        ...
        *_, final_global_batch = dataloader
        microbatch_iter, _ = prepare_finetuning_batch(
            iter([final_global_batch]),
            num_microbatches=global_batch_size // micro_batch_size,
            default_seq_length=dataset.max_seq_length,
        )

        autogenerated_rows = 0
        for microbatch in microbatch_iter:
            for row_index, metadata in enumerate(microbatch["metadata"]):
                if not metadata.get("__AUTOGENERATED__", False):
                    continue
                autogenerated_rows += 1
                row_start = int(microbatch["cu_seqlens_q"][row_index].item())
                row_end = int(microbatch["cu_seqlens_q"][row_index + 1].item())
                assert microbatch["loss_mask"][0, row_start:row_end].sum().item() == 0

        assert autogenerated_rows == global_batch_size - len(dataset) % global_batch_size
