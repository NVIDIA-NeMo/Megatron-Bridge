...
class _TinySFTSource:
    """Small source with the GPT SFT dataset surface used by blend tests."""
    ...
    def __getitem__(self, index):
        return {
            ...
            "loss_mask": [1, 1],
            "metadata": {"source": self.name, "index": index},
        }
...
def test_gpt_sft_blend_uses_relative_weights_and_default_total_size():
    blend = GPTSFTBlendDataset(
        [_TinySFTSource("a", 6), _TinySFTSource("b", 2)],
        [3.0, 1.0],
        seed=17,
    )

    rows = [blend[index] for index in range(len(blend))]

    assert len(rows) == 8
    assert [row["metadata"]["source"] for row in rows].count("a") == 6
    assert [row["metadata"]["source"] for row in rows].count("b") == 2


def test_gpt_sft_unweighted_blend_keeps_every_source_row_once():
    blend = GPTSFTBlendDataset(
        [_TinySFTSource("a", 3), _TinySFTSource("b", 1)],
        None,
        seed=17,
    )
    ...
    assert [row["metadata"]["source"] for row in rows].count("a") == 3
    assert [row["metadata"]["source"] for row in rows].count("b") == 1


def test_gpt_sft_blend_is_deterministic_and_repeats_source_local_passes():
    sources = [_TinySFTSource("a", 2), _TinySFTSource("b", 2)]
    first = GPTSFTBlendDataset(sources, [1.0, 1.0], size=12, seed=9)
    same = GPTSFTBlendDataset(sources, [1.0, 1.0], size=12, seed=9)
    other = GPTSFTBlendDataset(sources, [1.0, 1.0], size=12, seed=10)

    assert np.array_equal(first.dataset_index, same.dataset_index)
    assert np.array_equal(first.dataset_sample_index, same.dataset_sample_index)
    assert not np.array_equal(first.dataset_sample_index, other.dataset_sample_index)
    assert set(first.dataset_sample_index) == {0, 1}


def test_gpt_sft_blend_marks_negative_index_as_lossless_padding():
    blend = GPTSFTBlendDataset(
        [_TinySFTSource("a", 2), _TinySFTSource("b", 2)],
        [1.0, 1.0],
        seed=3,
    )
    ...
    assert sample["metadata"]["__AUTOGENERATED__"] is True
    assert blend.collate_fn([sample]) == {"batch": [sample]}
...
@pytest.mark.unit
@pytest.mark.parametrize("dataset_type", ["sft", "chat"])
@pytest.mark.parametrize("pad_to_max_length", [False, True])
def test_unpacked_padding_mask_preserves_real_tokens(tmp_path, dataset_type, pad_to_max_length):
    dataset, _ = get_gpt_sft(tmp_path, dataset_type=dataset_type, return_padding_mask=True)
    dataset.max_seq_length = 16
    dataset.pad_seq_length_to_mult = 8
    dataset.pad_to_max_length = pad_to_max_length
    eos = dataset.tokenizer.eos_id
    items = []
    for autogenerated, ids in [(False, [101, eos, 103, 104, eos]), (False, [201, 202, eos]), (True, [301, 302, eos])]:
        items.append(
            {
                "input_ids": np.array(ids) if dataset_type == "chat" else ids,
                "context_ids": np.array(ids[:2]) if dataset_type == "chat" else ids[:2],
                "answer_start_idx": 2,
                "context_length": 2,
                "answer_ids": np.array(ids[2:]) if dataset_type == "chat" else ids[2:],
                "loss_mask": np.array([0, 0] + [1] * (len(ids) - 2)),
                "metadata": {"__AUTOGENERATED__": True} if autogenerated else {},
                "token_count": len(ids),
            }
        )
    result = dataset.collate_fn(items)
    width = 16 if pad_to_max_length or dataset_type == "chat" else 8
    expected = torch.ones(3, width, dtype=torch.bool)
    expected[0, :4] = False
    expected[1, :2] = False
    assert torch.equal(result["padding_mask"], expected)
    assert result["tokens"].shape == expected.shape
    assert result["loss_mask"][0, 0] == 0
    assert not result["padding_mask"][0, 0]  # Prompt tokens still count for routing.
    assert result["tokens"][0, 1] == eos
    assert not result["padding_mask"][0, 1]  # A real EOS is not padding.
    assert result["labels"][0, 3] == eos
    assert result["loss_mask"][0, 3] == 1
    assert result["loss_mask"][result["padding_mask"]].sum() == 0
    assert not any(key.startswith("cu_seqlens") for key in result)
    dataset.return_padding_mask = False
    legacy = dataset.collate_fn(items)
    assert "padding_mask" not in legacy
    for key in ("tokens", "labels", "loss_mask", "position_ids"):
        assert torch.equal(legacy[key], result[key])


@pytest.mark.unit
def test_chat_padding_mask_handles_truncation_and_empty_shift(tmp_path):
    dataset, _ = get_gpt_sft(tmp_path, dataset_type="chat", return_padding_mask=True)
    dataset.max_seq_length = 4
    dataset.pad_to_max_length = True
    items = [
        {
            "input_ids": np.arange(8),
            "context_ids": np.array([0]),
            "answer_ids": np.arange(1, 8),
            "loss_mask": np.ones(8),
            "metadata": {},
        }
    ]
    ...
    assert dataset._build_padding_mask([{"input_ids": [2], "metadata": {}}], 4).all()
