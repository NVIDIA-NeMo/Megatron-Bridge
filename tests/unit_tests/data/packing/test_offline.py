...
def test_configured_seed_controls_offline_packing(monkeypatch):
    ...
    def prepare_with_ambient_seed(ambient_seed):
        ...
        def fake_save(file, arr, *args, **kwargs):
            # Let ragged store internals write to disk, but capture the final
            # output save from prepare_gpt_sft_packed_data.
            if str(file).endswith("unused.npy"):
...
def test_materialize_dataset_items_uses_serial_path_for_non_positive_workers(monkeypatch):
    ...
    class TinyDataset:
        ...
        def __getitem__(self, index):
            return {"input_ids": [index + 10, index + 11]}
    ...
    try:
        assert store.column("input_ids").get(0).tolist() == [10, 11]
        assert store.column("input_ids").get(2).tolist() == [12, 13]
    finally:
        store.close()

    store = _materialize_dataset_items(TinyDataset(), 0)
    try:
        assert store.column("input_ids").get(1).tolist() == [11, 12]
    finally:
        store.close()
...
def test_materialize_dataset_items_parallel_returns_correct_items():
    ...
    class TinyDataset:
        ...
        def __getitem__(self, index):
            return {"input_ids": [index + 100]}

    store = _materialize_dataset_items(TinyDataset(), 2)
    try:
        values = sorted(store.column("input_ids").get(i).tolist()[0] for i in range(store.num_items))
...
def test_materialize_dataset_items_preserves_dict_items():
    ...
    # Serial
    store = _materialize_dataset_items(TinyDataset(), num_workers=None)
    try:
        assert store.num_items == 4
        assert np.array_equal(store.column("input_ids").get(0), [0, 1])
        assert store.column("loss_mask").get(0).tolist() == [1, 0]
    finally:
        store.close()
    ...
    try:
        assert store.num_items == 4
        ids = {tuple(store.column("input_ids").get(i).tolist()) for i in range(store.num_items)}
...
def test_materialize_dataset_items_empty_dataset():
    """An empty dataset must produce an empty store, not raise."""
    ...
    try:
        ...
        assert len(store.input_ids_lengths()) == 0
    finally:
        store.close()
