...
class TestSliceBatchForMegatronMIMO:
    ...
    def test_sparse_metadata_keeps_dense_patch_packed_slicing(self):
        ...
        batch = {
            ...
            "modality_inputs": {
                "images": {"hidden_states": hidden_states, "grid_thw": grids},
                "audio": {"x": torch.tensor([[1], [3]])},
            },
        ...
        images = shard_1["modality_inputs"]["images"]
        assert images["hidden_states"].shape == (41, 1)
        assert set(images["hidden_states"].unique().tolist()) == {2, 3}
        torch.testing.assert_close(images["grid_thw"], grids[2:])
        torch.testing.assert_close(shard_1["modality_inputs"]["audio"]["x"], torch.tensor([[3]]))

    def test_sparse_ownership_after_encoder_adapter_nesting(self):
        batch = {
            "input_ids": torch.arange(8).reshape(4, 2),
            "labels": torch.arange(8).reshape(4, 2),
            "loss_mask": torch.ones(4, 2),
