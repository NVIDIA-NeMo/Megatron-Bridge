...
"""Configuration checks for MoE CUDA-graph performance recipes."""
...
def _assert_te_scoped_hybridep(cfg: ConfigContainer) -> None:
    assert cfg.model.cuda_graph_impl == "transformer_engine"
    assert cfg.model.cuda_graph_scope == ["attn", "mamba", "moe_router", "moe_preprocess"]
    assert cfg.model.use_te_rng_tracker is True
    assert cfg.rng.te_rng_tracker is True
    assert cfg.ddp.check_for_nan_in_grad is False
    assert cfg.rerun_state_machine.check_for_nan_in_loss is False
    ...
    assert cfg.model.moe_token_dispatcher_type == "flex"
    assert cfg.model.moe_shared_expert_overlap is False
    assert getattr(cfg.model, "moe_pad_experts_for_cuda_graph_inference", False) is False
...
def test_nemotron_3_super_gb200_mxfp8_uses_te_scoped_graphs() -> None:
    cfg = nemotron_3_super_pretrain_64gpu_gb200_fp8mx_config()

    _assert_te_scoped_hybridep(cfg)
...
def test_nemotron_3_super_gb200_nvfp4_uses_te_scoped_graphs() -> None:
    cfg = nemotron_3_super_pretrain_64gpu_gb200_nvfp4_config()
