...
"""Unit tests for OmniModal pretrain and setup wiring."""
...
@patch(
    "megatron.bridge.training.setup_omni_modal.is_current_rank_in_grid",
...
def test_set_omni_modal_random_seeds_calls_model_parallel_cuda_manual_seed(mock_dist, _mock_in_grid):
    """_set_omni_modal_random_seeds should derive TP/PP ranks from grids and call model_parallel_cuda_manual_seed."""
    from megatron.bridge.training.setup_omni_modal import _set_omni_modal_random_seeds
    ...
    omni_modal_infra = SimpleNamespace(module_to_grid_map={"vision": grid})
...
@patch("megatron.bridge.training.setup_omni_modal.dist")
def test_set_omni_modal_random_seeds_offsets_by_pp_rank(mock_dist, _mock_in_grid):
    ...
    omni_modal_infra = SimpleNamespace(module_to_grid_map={"llm": grid})
    cfg = SimpleNamespace(rng=SimpleNamespace(seed=42))
...
@patch("megatron.bridge.training.pretrain_omni_modal.setup_omni_modal")
@patch("megatron.bridge.training.pretrain_omni_modal.dist")
...
def test_pretrain_omni_modal_calls_setup_and_train(
    mock_runtime_update, mock_dist, mock_setup_omni_modal, mock_train_omni_modal, mock_finish
):
    """pretrain_omni_modal should call setup_omni_modal then train_omni_modal."""
    ...
    mock_setup_omni_modal.return_value = setup_output
    ...
    mock_setup_omni_modal.assert_called_once()
    mock_train_omni_modal.assert_called_once()
...
@patch("megatron.bridge.training.setup_omni_modal.dist")
def test_setup_omni_modal_asserts_when_constructor_fields_missing(
    mock_dist, mock_get_model_config, mock_unwrap_omni_modal_model
):
    """setup_omni_modal guardrail should fail when module_to_grid_map is missing at construction."""
    ...
    mock_unwrap_omni_modal_model.return_value = unwrapped_model
    ...
    with (
        ...
        with pytest.raises(AssertionError, match="module_to_grid_map must be set"):
            setup_omni_modal(state=mock_state)
