...
"""Unit tests for OmniModal checkpoint saving and loading wiring.

Tests validate that OmniModal training correctly uses shared checkpoint helpers
...
def _make_omni_modal_infra(*, num_active_pgs: int = 1) -> Mock:
...
def _make_global_state(
    *,
    ...
    """Create a minimal GlobalState-like namespace for train_omni_modal tests."""
    timer_handle = Mock()
...
# ---------------------------------------------------------------------------
# Tests: pretrain_omni_modal setup wiring
# ---------------------------------------------------------------------------


class TestPretrainOmniModalSetup:
    """Verify pretrain_omni_modal properly initializes checkpointing runtime."""

    @patch("megatron.bridge.training.setup_omni_modal.checkpoint_exists", return_value=False)
    @patch("megatron.bridge.training.setup_omni_modal.get_active_module_pg")
    @patch("megatron.bridge.training.setup_omni_modal.create_checkpoint_manager")
    @patch("megatron.bridge.training.setup_omni_modal.MultiModulePipelineCommunicator")
    @patch("megatron.bridge.training.setup_omni_modal.get_model_config")
    @patch("megatron.bridge.training.setup_omni_modal.validate_no_stub_ranks")
    @patch("megatron.bridge.training.setup_omni_modal.build_pg_collection_for_schedule")
    @patch("megatron.bridge.training.setup_omni_modal.get_module_to_grid_tuple")
    @patch("megatron.bridge.training.setup_omni_modal._update_omni_modal_model_config_funcs")
    @patch("megatron.bridge.training.setup_omni_modal.unwrap_omni_modal_model")
    @patch("megatron.bridge.training.setup_omni_modal.dist")
    def test_setup_omni_modal_initializes_checkpoint_manager(
        self,
        mock_dist,
        mock_unwrap,
        ...
        with (
            patch("megatron.bridge.training.setup_omni_modal._set_omni_modal_random_seeds"),
            ...
            result = setup_omni_modal(state=global_state)
    ...
    def test_pretrain_omni_modal_calls_runtime_config_update(self):
        """pretrain_omni_modal should call omni_modal_runtime_config_update before setup."""
        from megatron.bridge.training.pretrain_omni_modal import pretrain_omni_modal
        ...
        with (
            patch("megatron.bridge.training.pretrain_omni_modal.omni_modal_runtime_config_update") as m_runtime,
            patch("megatron.bridge.training.pretrain_omni_modal.setup_omni_modal", return_value=setup_output),
            patch("megatron.bridge.training.pretrain_omni_modal.train_omni_modal"),
            patch("megatron.bridge.training.pretrain_omni_modal._finish_train"),
            patch("megatron.bridge.training.pretrain_omni_modal.dist") as m_dist,
        ):
...
class TestNonColocatedGuard:
    """Verify the non-colocated topology assertion in train_omni_modal."""

    @patch("megatron.bridge.training.train_omni_modal.build_pg_collection_for_schedule", return_value=Mock(spec=[]))
    @patch("megatron.bridge.training.train_omni_modal.get_module_to_grid_tuple")
    @patch("megatron.bridge.training.train_omni_modal.prepare_forward_step_func")
    @patch("megatron.bridge.training.train_omni_modal.get_num_microbatches", return_value=1)
    ...
    def test_rejects_multiple_active_pgs(self, *_mocks):
        ...
        infra = _make_omni_modal_infra(num_active_pgs=2)
    ...
    def test_rejects_zero_active_pgs(self, *_mocks):
        ...
        infra = _make_omni_modal_infra(num_active_pgs=0)
        state = _make_global_state(train_iters=0)
...
# ---------------------------------------------------------------------------
# Tests: checkpoint_and_decide_exit integration in train_omni_modal
# ---------------------------------------------------------------------------


class TestTrainOmniModalCheckpointIntegration:
    """Verify train_omni_modal calls checkpoint_and_decide_exit with the right args."""

    @patch("megatron.bridge.training.train_omni_modal.checkpoint_and_decide_exit", return_value=False)
    @patch("megatron.bridge.training.train_omni_modal.train_step_omni_modal")
    @patch("megatron.bridge.training.train_omni_modal.build_pg_collection_for_schedule")
    @patch("megatron.bridge.training.train_omni_modal.get_module_to_grid_tuple")
    @patch("megatron.bridge.training.train_omni_modal.prepare_forward_step_func")
    @patch("megatron.bridge.training.train_omni_modal.get_num_microbatches", return_value=1)
    @patch("torch.distributed.get_rank", return_value=0)
    ...
    @patch("megatron.bridge.training.train_omni_modal.checkpoint_and_decide_exit", return_value=True)
    @patch("megatron.bridge.training.train_omni_modal.train_step_omni_modal")
    @patch("megatron.bridge.training.train_omni_modal.build_pg_collection_for_schedule")
    @patch("megatron.bridge.training.train_omni_modal.get_module_to_grid_tuple")
    @patch("megatron.bridge.training.train_omni_modal.prepare_forward_step_func")
    ...
    def test_async_finalize_called_at_top_of_loop(
        ...
        # The blocking shutdown call is now in _finish_train (pretrain_omni_modal.py).
        ...
        # _finish_train (pretrain_omni_modal.py), tested separately.
...
def _make_setup_output_for_load(
    *,
    ...
    """Create a OmniModalSetupOutput-like namespace suitable for pretrain_omni_modal load tests."""
    ...
    return SimpleNamespace(
        model=MagicMock(),
        omni_modal_infra=SimpleNamespace(
...
def _make_pretrain_cfg(
    *,
    ...
    """Create a ConfigContainer-like mock for pretrain_omni_modal tests."""
    cfg = MagicMock()
...
def _run_pretrain_omni_modal(
    *,
    ...
    """Run pretrain_omni_modal with full mocking and return all mock handles.

    pretrain_omni_modal is a thin orchestrator: runtime_config_update → setup_omni_modal →
    train_omni_modal → _finish_train.  Checkpoint loading, iterator construction, and
    MPU bridging are now handled inside setup_omni_modal, so we mock setup_omni_modal at the
    boundary and only verify the orchestration.

    Returns dict with keys: setup_omni_modal, train_omni_modal, build_data_iterators_fn.
    """
    from megatron.bridge.training.pretrain_omni_modal import pretrain_omni_modal
    ...
    with (
        patch("megatron.bridge.training.pretrain_omni_modal.train_omni_modal") as m_train,
        patch("megatron.bridge.training.pretrain_omni_modal.setup_omni_modal", return_value=setup_output) as m_setup,
        patch("megatron.bridge.training.pretrain_omni_modal.dist") as m_dist,
        patch("megatron.bridge.training.pretrain_omni_modal.omni_modal_runtime_config_update"),
        patch("megatron.bridge.training.pretrain_omni_modal._finish_train"),
    ):
        m_dist.get_rank.return_value = 0
        ...
        mocks["setup_omni_modal"] = m_setup
        mocks["train_omni_modal"] = m_train
...
# ---------------------------------------------------------------------------
# Tests: pretrain_omni_modal passes build_data_iterators_fn to setup_omni_modal
# ---------------------------------------------------------------------------


class TestPretrainOmniModalPassesBuildFn:
    """Verify pretrain_omni_modal forwards build_data_iterators_fn to setup_omni_modal."""

    def test_build_fn_forwarded_to_setup_omni_modal(self):
        cfg = _make_pretrain_cfg()
        build_fn = Mock(return_value=(iter([]), None))
        mocks = _run_pretrain_omni_modal(cfg=cfg, build_data_iterators_fn=build_fn)
        _, kwargs = mocks["setup_omni_modal"].call_args
...
class TestActiveModulePgGuard:
    ...
    def test_rejects_zero_active_pgs(self):
        from megatron.bridge.training.omni_modal_parallel_utils import get_active_module_pg
...
class TestTrainStateRestorationSmoke:
    ...
    def test_train_state_step_accessible_after_load(self):
        ...
        mocks = _run_pretrain_omni_modal(
            cfg=cfg,
            setup_output=setup_output,
        )

        # train_state is passed to train_omni_modal via global_state
        _, kwargs = mocks["train_omni_modal"].call_args
...
# ---------------------------------------------------------------------------
# Tests: pretrain_omni_modal orchestration (setup → train)
# ---------------------------------------------------------------------------


class TestPretrainOmniModalOrchestration:
    """Verify pretrain_omni_modal calls setup_omni_modal and train_omni_modal correctly."""

    def test_train_omni_modal_always_called(self):
        """pretrain_omni_modal should always invoke train_omni_modal after setup."""
        cfg = _make_pretrain_cfg()
        mocks = _run_pretrain_omni_modal(cfg=cfg)
        mocks["train_omni_modal"].assert_called_once()

    def test_iterators_from_setup_forwarded_to_train(self):
        """Data iterators from setup_output should be forwarded to train_omni_modal."""
        cfg = _make_pretrain_cfg()
        build_fn = Mock(return_value=(iter([]), None))
        mocks = _run_pretrain_omni_modal(cfg=cfg, build_data_iterators_fn=build_fn)
        # build_fn is passed to setup_omni_modal, which is mocked — check that
        # train_omni_modal receives setup_output's iterators
        mocks["train_omni_modal"].assert_called_once()
...
# ---------------------------------------------------------------------------
# Tests: setup_omni_modal checkpoint loading path
# ---------------------------------------------------------------------------


class TestSetupOmniModalCheckpointLoading:
    """Verify setup_omni_modal invokes load_checkpoint when checkpoints exist."""

    _SETUP_PATCHES = [
        "megatron.bridge.training.setup_omni_modal.get_active_module_pg",
        "megatron.bridge.training.setup_omni_modal.create_checkpoint_manager",
        "megatron.bridge.training.setup_omni_modal.MultiModulePipelineCommunicator",
        "megatron.bridge.training.setup_omni_modal.get_model_config",
        "megatron.bridge.training.setup_omni_modal.validate_no_stub_ranks",
        "megatron.bridge.training.setup_omni_modal.build_pg_collection_for_schedule",
        "megatron.bridge.training.setup_omni_modal.get_module_to_grid_tuple",
        "megatron.bridge.training.setup_omni_modal._update_omni_modal_model_config_funcs",
        "megatron.bridge.training.setup_omni_modal.unwrap_omni_modal_model",
        "megatron.bridge.training.setup_omni_modal.dist",
        "megatron.bridge.training.setup_omni_modal._set_omni_modal_random_seeds",
    ...
    def _run_setup(self, *, load_path=None, checkpoint_exists_return=False):
        """Run setup_omni_modal with mocks, return dict of mock handles."""
        from megatron.bridge.training.setup_omni_modal import setup_omni_modal
        ...
        with (
            ...
            m_dist = stack.enter_context(patch("megatron.bridge.training.setup_omni_modal.dist"))
            m_dist.get_rank.return_value = 0
            ...
            m_ckpt_exists = stack.enter_context(
                patch(
                    "megatron.bridge.training.setup_omni_modal.checkpoint_exists",
                    return_value=checkpoint_exists_return,
                )
            )
            m_load = stack.enter_context(patch("megatron.bridge.training.setup_omni_modal.load_checkpoint"))
            ...
            stack.enter_context(
                patch("megatron.bridge.training.setup_omni_modal.get_model_config", return_value=model_config)
            )
            stack.enter_context(patch("megatron.bridge.training.setup_omni_modal.unwrap_omni_modal_model"))

            result = setup_omni_modal(state=state)
...
# ---------------------------------------------------------------------------
# Tests: setup_omni_modal resume-aware iterator construction
# ---------------------------------------------------------------------------


class TestSetupOmniModalResumeIterators:
    """Verify setup_omni_modal builds iterators with train_state when resuming."""
    ...
    def test_train_state_passed_when_resuming(self):
        ...
        def _sig_fn(cfg, omni_modal_infra, *, train_state=None):
            pass
        ...
        with (
            patch("megatron.bridge.training.setup_omni_modal.dist") as m_dist,
            patch("megatron.bridge.training.setup_omni_modal._set_omni_modal_random_seeds"),
            patch("megatron.bridge.training.setup_omni_modal.validate_no_stub_ranks"),
            patch("megatron.bridge.training.setup_omni_modal.build_pg_collection_for_schedule"),
            patch("megatron.bridge.training.setup_omni_modal.get_module_to_grid_tuple"),
            ...
            setup_omni_modal(state=state, build_data_iterators_fn=build_fn)
    ...
    def test_raises_when_builder_lacks_train_state_on_resume(self):
        ...
        def legacy_builder(cfg, omni_modal_infra):
            return (iter([]), None)
