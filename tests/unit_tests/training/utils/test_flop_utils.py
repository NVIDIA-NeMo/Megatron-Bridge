...
class TestHybridMLAFlops:
    ...
    def test_mla_formula_for_direct_and_low_rank_q(self, q_lora_rank):
        ...
        if model.q_lora_rank is None:
            ...
            q_proj = model.q_lora_rank * (model.hidden_size + model.num_attention_heads * q_head_dim + 1)
        kv_proj = model.kv_lora_rank * (
            model.hidden_size + model.num_attention_heads * (model.qk_head_dim + model.v_head_dim) + 1
        )
        kv_proj += model.hidden_size * model.qk_pos_emb_head_dim
    ...
    def test_mla_output_gate_respects_projection_granularity(self):
        """Elementwise gating projects one gate per value element, unlike Ling's head-wise gate."""
        batch_size = 2
        headwise = self._config(gated_attention_proj_granularity="headwise")
        elementwise = self._config(gated_attention_proj_granularity="elementwise")
        model = headwise.model
        total_tokens = batch_size * model.seq_length
        expected_delta = 3 * 2 * total_tokens * model.hidden_size * model.num_attention_heads * (model.v_head_dim - 1)

        assert (
            num_floating_point_operations(elementwise, batch_size=batch_size)
            - num_floating_point_operations(headwise, batch_size=batch_size)
            == expected_delta
        )
