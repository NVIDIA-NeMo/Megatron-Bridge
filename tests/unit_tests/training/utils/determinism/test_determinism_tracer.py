...
def test_await_async_helper():
    ...
    w = _W()
    ct._await_async(w)
    assert w.waited  # async work handle -> waited
    ct._await_async(None)  # sync collective (None) -> no-op, no error


def test_async_collective_waited_before_staging(tmp_path, monkeypatch):
    """Regression: an async collective's output is incomplete until work.wait(); the tracer must
    wait before staging, so the staged digest reflects the COMPLETED value — not partial data
    (which previously showed up as a spurious first-divergence in collectives-only traces)."""
    ...
    completed = torch.arange(8, dtype=torch.float32)

    class _FakeWork:
        def wait(self):
            out.copy_(completed)  # completion writes the real value

    def _fake_all_gather(output, input, *a, **k):
        return _FakeWork()  # async: output NOT written yet, returns a Work handle

    monkeypatch.setattr(dist, "all_gather_into_tensor", _fake_all_gather, raising=False)

    ct.enable(out_dir=str(tmp_path))  # wraps the (now fake) collective
    try:
        ct.set_active(True, window=1)
        dist.all_gather_into_tensor(out, torch.zeros(8))  # routed through the wrapper
        ct.set_active(False)  # drain
    finally:
        ct.disable()

    stream = tmp_path / "stream_pp0_tp0_cp0_dp0.fp"
    records = [json.loads(line) for line in stream.read_text().splitlines() if '"_header"' not in line]
    rec = next(r for r in records if r["op"] == "all_gather_into_tensor")
    # digest must be of the COMPLETED tensor → proves _await_async ran before staging the output
    assert rec["output"][0]["digest"] == tensor_signature(completed).digest
    assert rec["output"][0]["digest"] != tensor_signature(torch.zeros(8)).digest
