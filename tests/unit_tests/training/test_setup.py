...
class TestValidateAndSetVocabSize:
    ...
    def test_different_tensor_parallel_sizes(self):
        """Test that different tensor parallel sizes affect padding validation."""
        # Test with TP=1
        result = _validate_and_set_vocab_size(
            model_vocab_size=32128,  # 32128 = 251 * 128, divisible by 128*1=128
            tokenizer_vocab_size=32004,
            tokenizer_padded_vocab_size=32128,
            make_vocab_size_divisible_by=128,
            tensor_model_parallel_size=1,
        )
        assert result == 32128

        # Test with TP=16 (more restrictive padding)
        with pytest.raises(ValueError, match="not properly padded for tensor parallelism"):
            _validate_and_set_vocab_size(
                model_vocab_size=32128,  # Not divisible by 128*16=2048
                tokenizer_vocab_size=32004,
                tokenizer_padded_vocab_size=32768,
                make_vocab_size_divisible_by=128,
    ...
    def test_divisibility_requirements(self):
        """Test different make_vocab_size_divisible_by values."""
        result = _validate_and_set_vocab_size(
            model_vocab_size=32512,  # 32512 = 508 * 64, divisible by 64*8=512
            tokenizer_vocab_size=32004,
            tokenizer_padded_vocab_size=32512,
            make_vocab_size_divisible_by=64,
            tensor_model_parallel_size=8,
        )
        assert result == 32512

    def test_edge_case_minimum_valid_vocab_size(self):
        """Test edge case where vocab_size equals tokenizer_vocab_size and is properly padded."""
        # Construct a scenario where tokenizer vocab size is already properly padded
        tokenizer_size = 32768  # Already divisible by 128*8=1024
        result = _validate_and_set_vocab_size(
            model_vocab_size=tokenizer_size,
            tokenizer_vocab_size=tokenizer_size,
            tokenizer_padded_vocab_size=tokenizer_size,
            make_vocab_size_divisible_by=128,
            tensor_model_parallel_size=8,
        )
        assert result == tokenizer_size
