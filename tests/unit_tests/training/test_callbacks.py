    ...
    def test_subclass_can_override_methods(self):
        ...
        class MyCallback(Callback):
            def on_train_start(self, context):
                """
                Called when training begins to notify the callback of the training start.
                
                Parameters:
                    context (CallbackContext): Context for the run, providing `state`, `model`, `user_state` (a mutable dict), and optional fields such as `optimizer`, `scheduler`, `loss_dict`, `grad_norm`, `skipped_iter`, and `total_loss_dict`.
                """
                call_log.append("train_start")

            def on_eval_end(self, context):
                """
                Hook invoked at the end of an evaluation run.
                
                Parameters:
                    context (CallbackContext): Contextual state for the evaluation, containing fields such as `state`, `model`, and `user_state` (plus optional fields like `optimizer`, `scheduler`, `loss_dict`, etc.).
                """
    ...
    def test_add_single_callback(self):
        ...
        class MyCallback(Callback):
            def on_train_start(self, context):
                ...
                Override to run custom logic when training begins. The default implementation does nothing.
                
                Parameters:
                    context (CallbackContext): Context object containing training state, model, and optional utilities (optimizer, scheduler, loss_dict, etc.).
                """
                pass
            ...
            def on_eval_end(self, context):
                ...
                Called after evaluation completes; subclasses can override this to perform teardown, logging, metrics aggregation, or mutate shared user state.
                
                Parameters:
                	context (CallbackContext): The contextual state for the callback, including `state`, `model`, and `user_state` (and optional fields like `optimizer`, `scheduler`, `loss_dict`, etc.).
                """
                pass
...
class TestCallbackManagerFire:
    ...
    def test_fire_respects_mixed_registration_order(self):
        ...
        class FirstCallback(Callback):
            def on_train_start(self, context):
                """
                Append the marker 'class_first' to the shared call_order list to record that this class's on_train_start hook was invoked.
                
                Parameters:
                    context (CallbackContext): The callback context for the current training run.
                """
        ...
        class LastCallback(Callback):
            def on_train_start(self, context):
                ...
                Parameters:
                    context: CallbackContext providing the current training state, model, and optional utilities (optimizer, scheduler, etc.).
                """
...
class TestUserStatePersistence:
    ...
    def test_user_state_persists_across_fires(self):
        ...
        def increment_counter(ctx):
            """
            Increment the integer stored under the "counter" key in the context's user_state.
            
            Parameters:
                ctx (CallbackContext): Callback context whose `user_state` dict will be updated. If `user_state["counter"]` is missing, it is initialized to 1.
            """
    ...
    def test_user_state_shared_across_different_events(self):
        ...
        def write_start_time(ctx):
            """
            Record a fixed start time value in the callback context's user_state.
            
            Parameters:
                ctx (CallbackContext): Context whose `user_state` dictionary will be updated. The function sets `ctx.user_state["start_time"]` to 100.
            """
            ctx.user_state["start_time"] = 100

        def read_start_time(ctx):
            """
            Store an `elapsed` value in the context's `user_state` computed from its `start_time`.
            
            Parameters:
                ctx (CallbackContext): Context whose mutable `user_state` must contain a numeric `start_time`; sets `user_state['elapsed']` to 200 minus that value.
            """
