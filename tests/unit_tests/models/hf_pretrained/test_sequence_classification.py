...
def test_load_model_uses_auto_model_for_sequence_classification() -> None:
    wrapper = PreTrainedSequenceClassification(
        model_name_or_path="Qwen/Qwen3.5-sequence-classification",
        device="cpu",
    ...
    with patch(
        "megatron.bridge.models.hf_pretrained.sequence_classification."
...
def test_multimodal_processors_are_inherited_as_optional_artifacts(tmp_path) -> None:
    wrapper = PreTrainedSequenceClassification(
        model_name_or_path="Qwen/Qwen3.5-sequence-classification",
        device="cpu",
    )
