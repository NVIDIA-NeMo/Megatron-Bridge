...
from megatron.bridge.models.deepseek.deepseek_v32_bridge import DeepSeekV32Bridge
...
class TestDeepSeekV32Bridge:
    """Test cases for DeepSeekV32Bridge."""

    @pytest.fixture
    def ds_v32_config(self):
        return {
    ...
    @pytest.fixture
    def mock_pretrained_v32(self, ds_v32_config):
        cfg = Mock(spec=list(ds_v32_config.keys()))
        for k, v in ds_v32_config.items():
            setattr(cfg, k, v)
    ...
    def test_registration(self):
        assert issubclass(DeepSeekV32Bridge, MegatronModelBridge)

    def test_provider_bridge_maps_config(self, mock_pretrained_v32):
        bridge = DeepSeekV32Bridge()
        provider = bridge.provider_bridge(mock_pretrained_v32)
        assert isinstance(provider, MLAModelProvider)
        assert provider.hidden_size == mock_pretrained_v32.config.hidden_size
        assert provider.num_attention_heads == mock_pretrained_v32.config.num_attention_heads
        assert provider.ffn_hidden_size == mock_pretrained_v32.config.intermediate_size
        assert provider.vocab_size == mock_pretrained_v32.config.vocab_size
        assert provider.layernorm_epsilon == mock_pretrained_v32.config.rms_norm_eps
        assert provider.rotary_base == mock_pretrained_v32.config.rope_theta
        assert provider.moe_aux_loss_coeff == 0.001
        assert provider.bf16 is True
        assert provider.params_dtype == torch.bfloat16

    def test_provider_bridge_dsa_config(self, mock_pretrained_v32):
        bridge = DeepSeekV32Bridge()
        provider = bridge.provider_bridge(mock_pretrained_v32)
        assert provider.experimental_attention_variant == "dsa"
        assert provider.dsa_indexer_head_dim == 128
        assert provider.dsa_indexer_n_heads == 64
        assert provider.dsa_indexer_topk == 2048
        assert provider.dsa_indexer_loss_coeff == 0.001

    def test_export_injects_inv_freq_for_layer(self, mock_pretrained_v32):
        bridge = DeepSeekV32Bridge()
        bridge.hf_config = mock_pretrained_v32.config
        mock_pretrained_v32.state = {"model.layers.1.self_attn.rotary_emb.inv_freq": torch.randn(1)}
        task = WeightConversionTask(
            param_name="decoder.layers.0.input_layernorm.weight",
            global_param_name="decoder.layers.0.input_layernorm.weight",
            mapping=Mock(),
        ...
        converted = {"model.layers.0.input_layernorm.weight": torch.randn(1)}
        result = bridge.maybe_modify_converted_hf_weight(task, dict(converted), mock_pretrained_v32.state)

        inv_key = "model.layers.0.self_attn.rotary_emb.inv_freq"
        expected = 1.0 / (
            mock_pretrained_v32.config.rope_theta
            ** (
                torch.arange(0, mock_pretrained_v32.config.qk_rope_head_dim, 2, dtype=torch.float32)
                / mock_pretrained_v32.config.qk_rope_head_dim
            )
        )
    ...
    def test_export_skips_inv_freq_for_non_layernorm(self, mock_pretrained_v32):
        bridge = DeepSeekV32Bridge()
        bridge.hf_config = mock_pretrained_v32.config
        mock_pretrained_v32.state = {"model.layers.1.self_attn.rotary_emb.inv_freq": torch.randn(1)}
        task = WeightConversionTask(
    ...
    def test_export_skips_inv_freq_when_not_expected(self, mock_pretrained_v32):
        bridge = DeepSeekV32Bridge()
        bridge.hf_config = mock_pretrained_v32.config
        mock_pretrained_v32.state = {}
        task = WeightConversionTask(
            param_name="decoder.layers.0.input_layernorm.weight",
            global_param_name="decoder.layers.0.input_layernorm.weight",
            mapping=Mock(),
