...
class TestQwen3VLModel:
    ...
    def test_forward_localizes_dense_context_parallel_inputs_before_mtp(self, monkeypatch):
        """Dense CP passes one rank-local sequence for MTP embedding and supervision."""
        monkeypatch.setattr(
            "megatron.bridge.models.qwen_vl.modelling_qwen3_vl.model.torch.cuda.nvtx.range_push",
            lambda *_args, **_kwargs: None,
        )
        ...
        expected_cp_indices = torch.tensor([0, 1, 6, 7])
        ...
        assert language_model.last_kwargs is not None
        assert torch.equal(language_model.last_kwargs["input_ids"], input_ids.index_select(1, expected_cp_indices))
        assert torch.equal(
            language_model.last_kwargs["position_ids"], position_ids.index_select(2, expected_cp_indices)
        )
        assert torch.equal(language_model.last_kwargs["labels"], labels.index_select(1, expected_cp_indices))
        assert torch.equal(language_model.last_kwargs["loss_mask"], loss_mask.index_select(1, expected_cp_indices))
        assert torch.equal(local_loss_mask, loss_mask.index_select(1, expected_cp_indices))
        assert torch.equal(output, torch.ones(1))
