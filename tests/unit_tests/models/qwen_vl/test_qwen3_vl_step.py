...
def test_forward_step_schedule_plan_receives_multimodal_inputs(monkeypatch):
    class _ProcessGroup:
        def rank(self):
            return 0
    ...
    config = type(
        "ModelConfig",
        (),
        {"mtp_num_layers": 0, "seq_length": 4, "overlap_moe_expert_parallel_comm": True},
    )()
    state = type(
        "State",
        (),
        {
            ...
            "cfg": type(
                ...
                (),
                {
                    "dataset": type("Dataset", (), {"enable_in_batch_packing": False})(),
                    "rerun_state_machine": type(
                        "Rerun", (), {"check_for_nan_in_loss": False, "check_for_spiky_loss": False}
                    )(),
                },
    ...
    multimodal_inputs = {"pixel_values": pixel_values, "image_grid_thw": image_grid_thw}
    model = _Model()
    ...
    monkeypatch.setattr(
        "megatron.bridge.models.qwen_vl.qwen3_vl_step.get_batch",
        lambda *args, **kwargs: (tokens, labels, loss_mask, attention_mask, position_ids, multimodal_inputs),
    )
    monkeypatch.setattr(
        "megatron.bridge.models.qwen_vl.qwen3_vl_step._pad_and_pack_qwen3_vl_step",
        lambda *args, **kwargs: (*args[:5], None),
    )
    monkeypatch.setattr(
        "megatron.bridge.models.qwen_vl.qwen3_vl_step.get_batch_on_this_cp_rank",
        lambda forward_args, **kwargs: forward_args,
    ...
    monkeypatch.setattr(
        "megatron.bridge.models.qwen_vl.qwen3_vl_step.accumulate_flops_metadata",
        lambda *args, **kwargs: None,
    )
    monkeypatch.setattr(
        "megatron.bridge.models.qwen_vl.qwen3_vl_step.get_model_chunk_vp_stage",
        lambda _: None,
    )

    plan, _ = forward_step(state, iter(()), model, return_schedule_plan=True)

    assert plan is not None
    assert model.schedule_kwargs is not None
    assert torch.equal(model.schedule_kwargs["input_ids"], tokens)
    assert torch.equal(model.schedule_kwargs["labels"], labels)
    assert torch.equal(model.schedule_kwargs["loss_mask"], loss_mask)
    assert model.schedule_kwargs["position_ids"] is None
    assert model.schedule_kwargs["pixel_values"] is pixel_values
    assert model.schedule_kwargs["image_grid_thw"] is image_grid_thw
