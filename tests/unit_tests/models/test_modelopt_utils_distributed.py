...
def _terminal_export(
    ...
    *,
    ...
    _export_quantized_weight(module, weight.dtype)
    attrs = quantizer_attr_names("weight")
    prefix = hf_name.removesuffix("weight")
    return {
        hf_name: module.weight.detach(),
        f"{prefix}{attrs.weight_scale}": getattr(module, attrs.weight_scale).detach(),
        f"{prefix}{attrs.weight_scale_2}": getattr(module, attrs.weight_scale_2).detach(),
        f"{prefix}{attrs.input_scale}": getattr(module, attrs.input_scale).detach(),
    }
...
@pytest.mark.gpu
def test_modelopt_export_tp2_pp2_ep2_matches_terminal_export(monkeypatch) -> None:
    ...
    try:
        ...
        expected_config = build_hf_quantization_config(
            {
                f"model.layers.{layer}.mlp.up_proj": QuantizedWeightExportState(
                    quantization_format=QUANTIZATION_NVFP4,
                    block_size=16,
                    weight_amax=torch.tensor(2.0 + layer),
                    input_amax=torch.tensor(3.0 + layer),
                )
        ...
        expert_hf_name = "model.layers.0.mlp.experts.gate_up_proj.weight"
        expert_task = _task(
            expert_module,
            f"decoder.layers.0.mlp.experts.linear_fc1.weight{rank}",
            expert_hf_name,
        ...
        canonical_expert_weight = torch.stack(gathered_expert_weights)
        bridge = object.__new__(MegatronModelBridge)
        ep_tensors = bridge._accumulate_grouped_export(
            ep_plan.conversion_tasks[0],
            {expert_hf_name: canonical_expert_weight},
            SimpleNamespace(num_moe_experts=_WORLD_SIZE),
            {},
            {},
        )
        assert ep_tensors is not None
        terminal_experts = [
            _terminal_export(
                expert_hf_name,
        ...
        expected_ep_tensors = {
            name: torch.stack([expert[name] for expert in terminal_experts]) for name in terminal_experts[0]
        }
