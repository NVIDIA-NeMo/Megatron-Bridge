...
"""
Unit tests for Qwen3OmniMoe Model implementation.

Run with: torchrun --nproc_per_node=8 -m pytest tests/unit_tests/models/qwen_omni/modeling_qwen3_omni/test_model.py
Or for single GPU: pytest tests/unit_tests/models/qwen_omni/modeling_qwen3_omni/test_model.py
"""
...
from transformers import AutoProcessor, Qwen3OmniMoeConfig
...
def processor():
    ...
    return AutoProcessor.from_pretrained("Qwen/Qwen3-Omni-30B-A3B-Instruct")


@pytest.fixture(scope="module")
def hf_config():
    """Load HuggingFace config once for all tests."""
    return Qwen3OmniMoeConfig.from_pretrained("Qwen/Qwen3-Omni-30B-A3B-Instruct")


@pytest.fixture
def random_image():
    """Generate a random PIL image."""
    return np.random.randint(0, 255, size=(24, 24, 3), dtype=np.uint8)


@pytest.fixture
def random_video():
    """Generate a random video."""
    return np.random.randint(0, 255, size=(2, 3, 24, 44), dtype=np.uint8)
...
class TestQwen3OmniMoeModel:
    ...
    @staticmethod
    def get_thinker_transformer_config(hf_config):
        """Create a thinker transformer config for testing.

        Returns:
            TransformerConfig: Configuration for the thinker model.
        """
        return hf_config.thinker_config

    @staticmethod
    def get_talker_transformer_config(hf_config):
        """Create a talker transformer config for testing.

        Returns:
            TransformerConfig: Configuration for the thinker model.
        """
        return hf_config.talker_config

    @staticmethod
    def get_code2wav_transformer_config(hf_config):
        """Create a code2wav transformer config for testing.
    ...
    def get_language_transformer_config(hf_config):
        ...
        Uses actual Qwen3-Omni-30B-A3B model sizes to ensure compatibility
        ...
        Returns:
            Qwen3OmniTransformerConfig: Configuration for the language model.
        """
        thinker_config = hf_config.thinker_config
        return Qwen3OmniTransformerConfig(
            # Use actual model dimensions from HF config
            num_layers=4,  # Reduced for testing (actual: thinker_config.text_config.num_hidden_layers)
            hidden_size=thinker_config.text_config.hidden_size,  # Must match vision output: 2048
            num_attention_heads=thinker_config.text_config.num_attention_heads,
            num_query_groups=thinker_config.text_config.num_key_value_heads,
            kv_channels=thinker_config.text_config.hidden_size // thinker_config.text_config.num_attention_heads,
            ffn_hidden_size=thinker_config.text_config.intermediate_size,
            # Qwen3-Omni specific
            vocab_size=thinker_config.text_config.vocab_size,
            language_max_sequence_length=thinker_config.text_config.max_position_embeddings,
            # Vision parameters
            ...
            out_hidden_size=thinker_config.text_config.hidden_size,  # Vision output = language input
            # RoPE settings
            rotary_base=thinker_config.text_config.rope_theta,
            rotary_percent=1.0,
            mrope_section=thinker_config.text_config.rope_scaling.get("mrope_section", [24, 20, 20]),
            hf_text_config=thinker_config.text_config,
            # Training settings
            ...
            add_qkv_bias=True,
            layernorm_epsilon=thinker_config.text_config.rms_norm_eps,
            bf16=False,
            use_cpu_initialization=True,
            hidden_dropout=0.0,
            attention_dropout=thinker_config.text_config.attention_dropout,
            num_moe_experts=2,  # Reduced for testing (actual: thinker_config.text_config.num_experts)
        )
    ...
    def get_language_model_layer_spec():
        ...
        language_model_layer_spec = get_gpt_layer_with_transformer_engine_spec(
            num_experts=2,  # Reduced for testing (actual: hf_config.thinker_config.text_config.num_experts)
    ...
    def get_data_batch(processor, random_image):
        ...
        Returns:
            dict: A dictionary containing all inputs needed for model forward pass:
                ...
                - input_feature: Audio values (None if no audio)
                - feature_attention_mask: Audio attention mask (None if no audio)
        ...
        messages = [
            {
                ...
                "content": [
                    ...
                    {"type": "text", "text": "Describe this image, video and aduio."},
                ],
            }
        ]
    ...
    def test_model_freeze_api(self, freeze_all, hf_config):
        ...
        assert pg_collection.ep is not None
        ...
        thinker_transformer_config = self.get_thinker_transformer_config(hf_config)
        talker_transformer_config = self.get_talker_transformer_config(hf_config)
        code2wav_transformer_config = self.get_code2wav_transformer_config(hf_config)

        model = Qwen3OmniMoeModel(
            language_transformer_config=language_transformer_config,
            language_transformer_layer_spec=language_model_layer_spec,
            thinker_transformer_config=thinker_transformer_config,
            talker_transformer_config=talker_transformer_config,
            code2wav_transformer_config=code2wav_transformer_config,
