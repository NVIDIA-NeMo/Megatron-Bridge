...
def _make_mlp(*, fused_pre_mlp_norm=True):
    ...
    return SimpleNamespace(linear_fc1=linear_fc1, linear_fc2=_linear(_FFN_HIDDEN_SIZE, _HIDDEN_SIZE, bias=False))


def _make_layer(*, explicit_pre_mlp_norm=False):
    pre_mlp_layernorm = _WeightOnlyNorm(_HIDDEN_SIZE) if explicit_pre_mlp_norm else SimpleNamespace()
    pre_mlp_layernorm_gen = _WeightOnlyNorm(_HIDDEN_SIZE) if explicit_pre_mlp_norm else SimpleNamespace()
    return SimpleNamespace(
...
def _make_model(num_layers=2, *, explicit_pre_mlp_norm=False):
    language_model = SimpleNamespace(
        ...
        decoder=SimpleNamespace(
            layers=[_make_layer(explicit_pre_mlp_norm=explicit_pre_mlp_norm) for _ in range(num_layers)],
    ...
    diffusion = SimpleNamespace(
        encoders={
            "timestep": SimpleNamespace(
                mlp=torch.nn.Sequential(_linear(3, _HIDDEN_SIZE), torch.nn.SiLU(), _linear(_HIDDEN_SIZE, _HIDDEN_SIZE))
            ),
            "latent_position_ids": SimpleNamespace(pos_embed=torch.nn.Parameter(torch.zeros(4, _HIDDEN_SIZE))),
        },
...
def _make_source_state(model, num_layers=2):
    ...
    def tensor(key, shape):
        ...
        state[key] = torch.arange(value_offset, value_offset + numel, dtype=torch.float32).reshape(shape)
        value_offset += numel
...
def test_full_native_checkpoint_maps_language_vision_and_auxiliary(tmp_path):
    ...
    for layer_index, layer in enumerate(model.language_model.decoder.layers):
        ...
        for source_suffix, destination_suffix in (("", ""), ("_moe_gen", "_gen")):
            ...
            assert all(torch.equal(actual, expected) for actual, expected in zip(actual_qkv, expected_qkv))
    ...
    assert torch.equal(diffusion.encoders["latent_position_ids"].pos_embed, source["latent_pos_embed.pos_embed"])
    assert torch.equal(diffusion.output_projections[0].weight, source["llm2vae.weight"])


def test_official_checkpoint_without_export_metadata_maps_all_tensors(tmp_path):
    model = _make_model()
    source = _make_source_state(model)
    ...
    assert report.source_tensors_consumed == len(source)
    assert torch.equal(
        model.language_model.embedding.word_embeddings.weight,
        source["language_model.model.embed_tokens.weight"],
    )
