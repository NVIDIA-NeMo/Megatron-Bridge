...
class TestParallelLinearAdapter:
    ...
    def test_parallel_linear_adapter_init_column_input(self, mock_row_linear, mock_col_linear, mock_config):
        ...
        mock_row_linear.return_value = mock_linear_out
    ...
    def test_parallel_linear_adapter_get_activation_fn(self, mock_config):
        """Test activation function selection."""
        adapter = ParallelLinearAdapter(
            in_features=10, out_features=5, dim=4, base_linear_name="test",
            activation="relu", model_parallel_config=mock_config
        )
        ...
        for activation_name, expected_type in activations_to_test.items():
            adapter = ParallelLinearAdapter(
                in_features=10, out_features=5, dim=4, base_linear_name="test",
                activation=activation_name, model_parallel_config=mock_config
            )
            assert isinstance(adapter.activation, expected_type)
    
    def test_parallel_linear_adapter_get_init_fn(self, mock_config):
        """Test initialization function selection."""
        adapter = ParallelLinearAdapter(
            in_features=10, out_features=5, dim=4, base_linear_name="test",
            column_init_method="xavier", model_parallel_config=mock_config
        )
    ...
    def test_parallel_linear_adapter_alpha_parameter(self, mock_config):
        """Test alpha parameter handling."""
        # Test default alpha (equals dim)
        adapter1 = ParallelLinearAdapter(
            in_features=10, out_features=5, dim=8, base_linear_name="test",
            model_parallel_config=mock_config
        )
        ...
        adapter2 = ParallelLinearAdapter(
            in_features=10, out_features=5, dim=8, base_linear_name="test",
            alpha=16, model_parallel_config=mock_config
        )
    ...
    def test_parallel_linear_adapter_dropout(self, mock_row_linear, mock_col_linear, mock_config):
        ...
        adapter1 = ParallelLinearAdapter(
            in_features=10, out_features=5, dim=4, base_linear_name="test",
            dropout=0.0, model_parallel_config=mock_config
        )
        ...
        adapter2 = ParallelLinearAdapter(
            in_features=10, out_features=5, dim=4, base_linear_name="test",
            dropout=0.3, model_parallel_config=mock_config
        )
    ...
    def test_parallel_linear_adapter_forward_basic(self, mock_row_linear, mock_col_linear, mock_config):
        ...
        adapter = ParallelLinearAdapter(
            in_features=20, out_features=10, dim=16, base_linear_name="test",
            input_is_parallel=False, model_parallel_config=mock_config
        )
    ...
    def test_parallel_linear_adapter_expert_mode(self, mock_row_linear, mock_col_linear, mock_config):
        ...
        adapter = ParallelLinearAdapter(
            in_features=20, out_features=10, dim=16, base_linear_name="test",
            is_expert=True, model_parallel_config=mock_config
        )
        
        # Test with sequence length that needs padding
        x = torch.randn(7, 20)  # Will be padded to 8 (assuming tp_size=1)
        output = adapter(x)
    ...
    def test_parallel_linear_adapter_sharded_state_dict(self, mock_row_linear, mock_col_linear, mock_config):
        ...
        adapter = ParallelLinearAdapter(
            in_features=20, out_features=10, dim=16, base_linear_name="linear_fc2",
            model_parallel_config=mock_config
        )
    ...
    def test_parallel_linear_adapter_sharded_state_dict_fc1_special_case(
        ...
        adapter = ParallelLinearAdapter(
            in_features=20, out_features=10, dim=16, base_linear_name="linear_fc1",
            model_parallel_config=mock_config
        )
