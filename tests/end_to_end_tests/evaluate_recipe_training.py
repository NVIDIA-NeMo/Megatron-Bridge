...
def validate_performance(
    current_step_timing_values: np.ndarray,
...
def write_golden_values_to_disk(current_values: Dict[str, Any], golden_values_path: str):
    """
...
def calc_convergence(
    ...
    write_golden_values_to_disk(
        {**current_train_loss, **current_step_timing},
        golden_values_path=os.path.join(assets_dir, "golden_values", golden_values_file_name),
    )
    ...
    logger.info(f"Comparing {len(steps)} training steps for convergence")
    logger.info(f"Current timing values: {current_step_timing_values}")
    logger.info(f"Golden timing values: {golden_step_timing_values}")

    performance_result = validate_performance(
        current_step_timing_values, golden_step_timing_values, steps, logger, performance_config
    )
    ...
    if wandb_run:
        ...
        with artifact.new_file("golden_values.json", "w") as f:
            json.dump({datetime.now().strftime("%m.%d.%y"): {**current_train_loss, **current_step_timing}}, f)
