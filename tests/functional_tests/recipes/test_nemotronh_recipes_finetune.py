...
class TestNemotron3NanoFinetuneRecipes:
    ...
    _TOY_MODEL_ID = "nvidia/NVIDIA-Nemotron-3-Nano-30B-A3B-Base-BF16"
    ...
    @pytest.fixture(scope="class")
    def nemotron_3_nano_megatron_checkpoint(self, nemotron_3_nano_toy_model_path: str, tmp_path_factory: pytest.TempPathFactory) -> str:
        """
        Convert the toy HuggingFace model to Megatron checkpoint format.
    ...
    def _get_finetune_config(self, checkpoint_dir: str, peft: Optional[str] = None, **kwargs) -> Any:
        """
    ...
    def test_nemotron_3_nano_finetune_recipes(
        ...
        config = self._get_finetune_config(
            checkpoint_dir=nemotron_3_nano_megatron_checkpoint,
            dir=str(tmp_path),
            name=recipe_name,
            peft=peft_strategy
        )
