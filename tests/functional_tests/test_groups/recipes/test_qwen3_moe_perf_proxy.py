...
The existing Qwen recipe functional tests cover small dense models.  These
proxies instead start from the production MoE performance constructors and
reduce the layer count (plus H100 EP size) to fit one eight-GPU runner.  The
shared functional helper also shortens the sequence and batch sizes.  The tests
retain the model width, expert count, top-k, precision, HybridEP, and CUDA-graph
behavior that are otherwise first exercised in large-scale CI.  A null
tokenizer keeps the mock-data test offline without changing the model path.
"""
...
def _qwen3_moe_proxy(
    ...
    *,
    ...
    # Guard the production behaviors this proxy is intended to exercise.  Do
    # not let later edits silently turn it into another small dense-Qwen test.
...
class TestQwen3MoePerfProxy:
    """Train reduced production configs on matching eight-GPU CI runners."""
