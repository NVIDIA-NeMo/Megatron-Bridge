...
def _copy_custom_code_from_hub(repo_id: str, model_dir: Path, source_file: str | Path) -> None:
...
class TestNemotronHConversion:
    ...
    def nemotronh_toy_model_path(self, tmp_path_factory):
        ...
        _copy_custom_code_from_hub("nvidia/Nemotron-H-8B-Base-8K", model_dir, modeling_filepath)
...
class TestNemotron3NanoConversion:
    ...
    def nemotron_3_nano_toy_model_path(self, tmp_path_factory):
        ...
        model.save_pretrained(model_dir, safe_serialization=True, save_original_format=False)
        modeling_filepath = os.path.abspath(sys.modules[model_class.__module__].__file__)
        _copy_custom_code_from_hub(repo_id, model_dir, modeling_filepath)
...
class TestNemotron3SuperConversion:
    """
    Test Nemotron-3-Super MoE model conversion from local HuggingFace model with different parallelism configurations.

    This test class tests the nvidia/NVIDIA-Nemotron-3-Super-120B-A12B-BF16 model which is a Latent Mixture-of-Experts
    (LatentMoE) variant of the NemotronH architecture with Multi-Token Prediction (MTP) layers.
    """

    _TOY_MODEL_ID = "nvidia/NVIDIA-Nemotron-3-Super-120B-A12B-BF16"

    @pytest.fixture(scope="class")
    def nemotron_3_super_toy_model_path(self, tmp_path_factory):
        """
        ...
        # Create Nemotron-3-Super toy model config by starting with the HF model and applying overrides
        config = AutoConfig.from_pretrained(repo_id, trust_remote_code=True)
        ...
        # Download and save tokenizer from the reference Nemotron-3-Super model
        tokenizer = AutoTokenizer.from_pretrained(repo_id, trust_remote_code=True)
        tokenizer.save_pretrained(model_dir)
    ...
    def test_toy_model_creation(self, nemotron_3_super_toy_model_path, temp_hf_modules):
        """
        Test that the Nemotron-3-Super toy model is created correctly and can be loaded.

        Args:
            nemotron_3_super_toy_model_path: Path to the toy Nemotron-3-Super model (from fixture)
            temp_hf_modules: Temporary HF modules cache path (from fixture)
        """
        # Verify the model directory exists
        model_path = Path(nemotron_3_super_toy_model_path)
        assert model_path.exists(), f"Model directory not found at {model_path}"
        ...
        # Verify core model configuration
        assert config_data["model_type"] == "nemotron_h"
        for keys in HF_NEMOTRON_3_SUPER_TOY_MODEL_OVERRIDES.keys():
            assert config_data[keys] == HF_NEMOTRON_3_SUPER_TOY_MODEL_OVERRIDES[keys]

        # Try loading the model to verify it's valid
        try:
            model = AutoModelForCausalLM.from_pretrained(
                nemotron_3_super_toy_model_path,
                torch_dtype=torch.bfloat16,
                low_cpu_mem_usage=False,  # Ensure full loading
                trust_remote_code=True,
                cache_dir=nemotron_3_super_toy_model_path,
            )

            # Try loading the tokenizer as well
            try:
                tokenizer = AutoTokenizer.from_pretrained(nemotron_3_super_toy_model_path, trust_remote_code=True)
                print(f"Tokenizer loaded successfully with vocab_size: {tokenizer.vocab_size}")
            except Exception as e:
                print(f"Warning: Could not load tokenizer (this might be OK for conversion testing): {e}")
            ...
            assert len(model.model.layers) == len(
                HF_NEMOTRON_3_SUPER_TOY_MODEL_OVERRIDES["layers_block_type"]
            )  # num_hidden_layers from toy config

            print(f"SUCCESS: Nemotron-3-Super toy model created and validated at {nemotron_3_super_toy_model_path}")

        except Exception as e:
            assert False, f"Failed to load created Nemotron-3-Super toy model: {e}"
    ...
    def test_nemotron_3_super_conversion_parallelism(
        self, nemotron_3_super_toy_model_path, tmp_path, tp, pp, test_name, temp_hf_modules
    ):
        """
        Test Nemotron-3-Super MoE model conversion with different parallelism configurations.
        ...
        # Create temporary output directory for conversion results
        test_output_dir = tmp_path / f"nemotron_3_super_{test_name}"
        test_output_dir.mkdir(exist_ok=True)
        ...
        cmd = [
            ...
            "examples/conversion/hf_megatron_roundtrip_multi_gpu.py",
            "--hf-model-id",
            nemotron_3_super_toy_model_path,  # Use our local toy model instead of downloading
            "--output-dir",
            str(test_output_dir),
            "--tp",
            str(tp),
        ...
        try:
            ...
            if result.returncode != 0:
                print(f"STDOUT: {result.stdout}")
                print(f"STDERR: {result.stderr}")
                assert False, f"Nemotron-3-Super {test_name} conversion failed with return code {result.returncode}"

            # Verify that the converted model was saved
            model_name = Path(nemotron_3_super_toy_model_path).name
            converted_model_dir = test_output_dir / model_name
            assert converted_model_dir.exists(), f"Converted model directory not found at {converted_model_dir}"
            ...
            # Verify the config contains Nemotron-3-Super specific parameters
            with open(config_file) as f:
                saved_config = json.load(f)
            ...
            assert saved_config["model_type"] == "nemotron_h", "Model type should be nemotron_h"
            for keys in HF_NEMOTRON_3_SUPER_TOY_MODEL_OVERRIDES.keys():
                assert saved_config[keys] == HF_NEMOTRON_3_SUPER_TOY_MODEL_OVERRIDES[keys], (
                    f"{keys} should match toy config"
                )

            print(f"SUCCESS: Nemotron-3-Super {test_name} conversion test completed successfully")
            print(f"Converted model saved at: {converted_model_dir}")

        except Exception as e:
            print(f"Error during Nemotron-3-Super {test_name} conversion test: {e}")
            raise
