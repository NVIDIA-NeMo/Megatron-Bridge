...
class TestPretrain:
    ...
    def test_pretrain_with_eval_at_start(self, tmp_path, capsys):
        ...
        regular in-loop eval at iteration 5, whose cadence must stay unchanged.
        """
        initialize_distributed()
        shared_base_dir = broadcast_path(tmp_path)
        ...
        try:
            ...
            if torch.distributed.get_rank() == torch.distributed.get_world_size() - 1:
                pre_train_marker = " validation loss at iteration 0 (pre-train validation) |"
                in_loop_marker = " validation loss at iteration 5 |"
                assert captured.count(pre_train_marker) == 1, captured
                assert in_loop_marker in captured, captured
                assert captured.index(pre_train_marker) < captured.index(in_loop_marker)
        finally:
            clear_directories(tmp_path)
