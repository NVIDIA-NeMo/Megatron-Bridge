...
def compare_provider_configs(converted_provider, predefined_provider, model_id):
    ...
    if missing_in_converted:
        raise AssertionError(
            f"Converted provider for {model_id} is missing attributes: {missing_in_converted}"
        )
    
    if missing_in_predefined:
        raise AssertionError(
            f"Predefined provider for {model_id} is missing attributes: {missing_in_predefined}"
        )
    ...
    for attr_name in sorted(converted_keys):
        ...
        if converted_value != predefined_value:
            ...
            if callable(converted_value) and callable(predefined_value):
                if (hasattr(converted_value, '__name__') and hasattr(predefined_value, '__name__') 
            ...
            mismatched_attrs.append(
                f"  {attr_name}: converted={converted_value} vs predefined={predefined_value}"
            )
    
    if mismatched_attrs:
        raise AssertionError(
            f"Configuration mismatch for {model_id}:\n" + "\n".join(mismatched_attrs)
        )
...
class TestQwenModelProviderMapping:
    ...
    @pytest.mark.parametrize(
        ...
        list(HF_MODEL_ID_TO_BRIDGE_MODEL_PROVIDER.items())
    )
