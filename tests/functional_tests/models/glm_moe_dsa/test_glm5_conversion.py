...
def _build_roundtrip_cmd(model_path: str, output_dir: Path, tp: int, pp: int, ep: int, repo_root: Path) -> list[str]:
    cmd = [
        sys.executable,
        "-m",
...
class TestGLM5Conversion:
    ...
    def test_toy_model_creation(self, glm5_toy_model_path):
        ...
        print(
            f"second_layer mlp experts: {second_layer.mlp.experts} and type: {type(second_layer.mlp.experts)} and size: {total_size} and shapes: {total_shapes}"
        )
    ...
    def test_glm5_conversion_parallelism(self, glm5_toy_model_path, tmp_path, tp, pp, ep, test_name):
        ...
        cmd = _build_roundtrip_cmd(glm5_toy_model_path, test_output_dir, tp, pp, ep, repo_root)
    ...
    def test_glm5_conversion_parallelism_local_model(self, tmp_path, tp, pp, ep, test_name):
        ...
        repo_root = _repo_root()
        print(
            f"model_dir: {model_dir}, test_output_dir: {test_output_dir}, repo_root: {repo_root}, tp: {tp}, pp: {pp}, ep: {ep}"
        )
        ...
        if result.returncode != 0:
            print(f"STDOUT: {result.stdout}")
            print(f"STDERR: {result.stderr}")
            assert False, f"GLM 5 MoE local-path {test_name} conversion failed with return code {result.returncode}"
