...
class TestMambaConversion:
    """
    Test Mamba model conversion from local HuggingFace model with different parallelism configurations.
    """

    @pytest.fixture(scope="class")
    def mamba_toy_model_path(self, tmp_path_factory):
        """
        Create and save a HuggingFace Mamba toy model from config to a temporary directory.
        ...
        temp_dir = tmp_path_factory.mktemp("mamba_toy_model")
        model_dir = temp_dir / "mamba_toy"

        # Create Mamba config from the toy model config
        config = MambaConfig(**HF_MAMBA_TOY_MODEL_CONFIG)
        ...
        # Download and save tokenizer from a reference Mamba model
        ...
        config_to_save = HF_MAMBA_TOY_MODEL_CONFIG.copy()
    ...
    def test_toy_model_creation(self, mamba_toy_model_path):
        """
        Test that the toy model is created correctly and can be loaded.

        Args:
            mamba_toy_model_path: Path to the toy Mamba model (from fixture)
        """
        # Verify the model directory exists
        model_path = Path(mamba_toy_model_path)
        ...
        try:
            ...
            try:
                tokenizer = AutoTokenizer.from_pretrained(mamba_toy_model_path)
    ...
    def test_mamba_conversion_parallelism(self, mamba_toy_model_path, tmp_path, tp, pp, test_name):
        ...
        test_output_dir = tmp_path / f"mamba_{test_name}"
        ...
        cmd = [
            ...
            "--hf-model-id",
            mamba_toy_model_path,  # Use our local toy model instead of downloading
            "--output-dir",
            str(test_output_dir),
            "--tp",
            str(tp),
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                assert False, f"Mamba {test_name} conversion failed with return code {result.returncode}"

            # Verify that the converted model was saved
            # The output directory should be named after the last part of the model path
            model_name = Path(mamba_toy_model_path).name  # "mamba_toy"
            ...
            # Verify the config contains Mamba-specific parameters
            with open(config_file) as f:
                saved_config = json.load(f)

            assert saved_config["model_type"] == "mamba", "Model type should be mamba"
            assert saved_config["hidden_size"] == 768, "Hidden size should match toy config"
            assert saved_config["num_hidden_layers"] == 2, "Number of hidden layers should match toy config"

            print(f"SUCCESS: Mamba {test_name} conversion test completed successfully")
            print(f"Converted model saved at: {converted_model_dir}")

        except Exception as e:
            print(f"Error during Mamba {test_name} conversion test: {e}")
            raise
