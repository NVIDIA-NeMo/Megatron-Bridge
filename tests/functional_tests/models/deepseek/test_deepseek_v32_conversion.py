...
class TestDeepSeekV32Conversion:
    """Functional tests for DeepSeek-V3.2 toy conversion paths."""

    @pytest.fixture(scope="class")
    def deepseek_toy_model_path(self, tmp_path_factory):
        temp_dir = tmp_path_factory.mktemp("deepseek_v32_toy_model")
        model_dir = temp_dir / "deepseek_v32_toy"

        # Create a minimal config and model using Auto classes to avoid direct imports
        config = AutoConfig.from_pretrained("deepseek-ai/DeepSeek-V3.2-Exp", trust_remote_code=True)
    ...
    def test_deepseek_conversion_parallelism(self, deepseek_toy_model_path, tmp_path, tp, pp, ep, test_name):
        test_output_dir = tmp_path / f"deepseek_v32_{test_name}"
        ...
        assert saved["model_type"] == "deepseek_v32", "Model type should be deepseek_v32"
