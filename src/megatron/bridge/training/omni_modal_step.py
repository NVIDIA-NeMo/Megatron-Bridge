...
"""OmniModal-specific forward step function for use with pipeline schedules.

This module provides the forward step function for OmniModal model training.
...
from megatron.bridge.data.omni_modal.dp_utils import slice_batch_for_omni_modal
from megatron.bridge.training.omni_modal_parallel_utils import unwrap_omni_modal_model
...
def _get_module_dp_info(
    ...
    Used to slice the global micro-batch via :func:`slice_batch_for_omni_modal`.
    Returns (0, 1) when grids are not configured (colocated mode).
    """
    grids = getattr(omni_modal_model.mimo_config, "module_to_grid_map", None)
    if not grids:
        return 0, 1
...
def loss_func(loss_mask: torch.Tensor, output_tensor: torch.Tensor) -> Tuple:
    """Loss function for OmniModal model training.
...
def get_batch(data_iterator: Iterable) -> Optional[Dict[str, torch.Tensor]]:
    ...
    Uses existing OmniModalDataset format from Phase 3.
...
def forward_step(
    ...
    """Forward step for OmniModal model training.
    ...
    needs_data = True
    if omni_modal_model.role is not None:
        if omni_modal_model.role.has_language_module:
            is_first_stage = omni_modal_model.role.is_first_stage(MIMO_LANGUAGE_MODULE_KEY)
            is_last_stage = omni_modal_model.role.is_last_stage(MIMO_LANGUAGE_MODULE_KEY)
            needs_data = is_first_stage or is_last_stage
        elif omni_modal_model.role.has_modality_modules:
            modality_modules = omni_modal_model.role.modality_module_names
            needs_data = any(omni_modal_model.role.is_first_stage(mod) for mod in modality_modules)
    ...
    if needs_data:
        ...
        # slice_batch_for_omni_modal contiguously sub-shards to match the
        # BridgeCommunicator's fan-in/fan-out batch-dimension routing.
        dp_rank, dp_size = _get_module_dp_info(omni_modal_model)
        data_batch = slice_batch_for_omni_modal(data_batch, dp_rank, dp_size)
    else:
    ...
    # omni_modal_model was already unwrapped at the start of this function
