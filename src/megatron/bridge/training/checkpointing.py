        ...
        try:
            if (
                ...
                if isinstance(optimizer, LayerWiseDistributedOptimizer) and ckpt_type == CheckpointType.LOCAL:
                    ...
                    # separate per-rank file (layer_wise_optimizer_<dp_rank>.pt) at the
                    # base checkpoint directory rather than embedding it in the sharded
                    # distributed checkpoint.  Load it back from that file here.
                    dp_rank = pg_collection.dp.rank()
                    optim_ckpt_path = os.path.join(load_dir, f"layer_wise_optimizer_{dp_rank}.pt")
