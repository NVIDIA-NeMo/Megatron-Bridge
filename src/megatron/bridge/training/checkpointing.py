...
def _get_run_config_model_parallel_sizes(model_config: dict[str, Any]) -> tuple[int, int]:
    """Return TP and PP sizes from legacy or builder-backed model config data."""
...
def _load_checkpoint_from_path(
    ...
    if ckpt_format == "torch_dist":
        ...
        if _is_megatron_mimo:
            ...
            ckpt_tp_pp = _get_run_config_model_parallel_sizes(run_config["model"])
            run_tp_pp = (
