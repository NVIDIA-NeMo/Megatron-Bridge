...
_SHARED_EXPERT_GLU_INTERLEAVE_SIZE = 32
...
def _is_swiglu_fc1_checkpoint_key(
    ...
    *,
    ...
    Shared experts are enabled separately by ``use_grouped_gemm_for_shared_expert`` or
    ``USE_GROUPED_GEMM_FOR_SHARED_EXPERT=1`` before this helper is called.
    """
    is_swiglu_fc1_dense = (
        include_dense
...
def _get_model_glu_interleave_sizes(
    ...
    use_grouped_gemm_for_shared_expert = getattr(
        model_config, "use_grouped_gemm_for_shared_expert", False
    ) or os.environ.get("USE_GROUPED_GEMM_FOR_SHARED_EXPERT", "0") == "1"
    shared_interleave_size = (
        _SHARED_EXPERT_GLU_INTERLEAVE_SIZE if use_grouped_gemm_for_shared_expert else None
    )
...
def _load_checkpoint_from_path(
    ...
    if not skip_load_to_model_and_opt:
        ...
        routed_interleave_size, shared_interleave_size = _get_model_glu_interleave_sizes(
            model, cfg
        )
