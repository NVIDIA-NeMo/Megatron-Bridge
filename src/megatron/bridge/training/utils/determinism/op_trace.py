...
# Per-OpOverload metadata memo: the ATen op set in a run is small and fixed, so ``str(func)``,
# the empty/c10d skip check, and the in-place name parse run once per distinct op — not once per
# dispatch (~500K/iter at op level).
_OP_META: dict = {}


def _op_meta(func):
    """Return memoized ``(name, skip, inplace)`` for an OpOverload ``func``."""
    m = _OP_META.get(func)
    if m is None:
        name = str(func)
