...
def training_log(
    ...
    if writer and (iteration % logger_config.tensorboard_log_interval == 0):
        ...
        if logger_config.log_throughput_to_tensorboard:
            ...
            if wandb_writer:
                wandb_writer.log(throughput_report, step=iteration)
        if logger_config.log_memory_to_tensorboard:
            ...
            for metric, value in memory_report.items():
                writer.add_scalar(metric, value, iteration)
            if wandb_writer:
                wandb_writer.log(memory_report, step=iteration)
        if logger_config.log_runtime_to_tensorboard:
            ...
            for metric, value in runtime_report.items():
                writer.add_scalar(metric, value, iteration)
            if wandb_writer:
                wandb_writer.log(runtime_report, step=iteration)
        if logger_config.log_l2_norm_grad_to_tensorboard:
            l2_report = report_l2_norm_grad(model)
            for metric, value in l2_report.items():
                writer.add_scalar(metric, value, iteration)
            if wandb_writer:
                wandb_writer.log(l2_report, step=iteration)
        if wandb_writer:
            wandb_writer.log({"samples vs steps": train_state.consumed_train_samples}, step=iteration)
        writer.add_scalar("learning-rate", learning_rate, iteration)
        writer.add_scalar("learning-rate vs samples", learning_rate, train_state.consumed_train_samples)
        if wandb_writer:
            wandb_writer.log({"learning-rate": learning_rate}, step=iteration)
        ...
        if logger_config.log_loss_scale_to_tensorboard:
            ...
            if wandb_writer:
                wandb_writer.log({"loss-scale": loss_scale}, step=iteration)
        if logger_config.log_world_size_to_tensorboard:
            ...
            if wandb_writer:
                wandb_writer.log({"world-size": get_world_size_safe()}, step=iteration)
        if grad_norm is not None:
            writer.add_scalar("grad-norm", grad_norm, iteration)
            writer.add_scalar("grad-norm vs samples", grad_norm, global_state.train_state.consumed_train_samples)
            if wandb_writer:
                wandb_writer.log({"grad-norm": grad_norm}, step=iteration)
    ...
    if iteration % logger_config.log_interval == 0:
        ...
        if logger_config.log_throughput_to_tensorboard:
            if writer:
                ...
                if wandb_writer:
                    wandb_writer.log({"throughput/tflops/device": per_gpu_tf}, step=iteration)
                    wandb_writer.log({"throughput/tflops": per_gpu_tf * get_world_size_safe()}, step=iteration)

        if logger_config.log_timers_to_tensorboard:
            if writer:
                writer.add_scalar("iteration-time", elapsed_time_per_iteration, iteration)
            if wandb_writer:
                wandb_writer.log({"iteration-time": elapsed_time_per_iteration}, step=iteration)
        ...
        if energy_monitor is not None:
            ...
            if wandb_writer:
                wandb_writer.log({"iter-energy/gpu": energy}, step=iteration)
                wandb_writer.log({"power/gpu": power}, step=iteration)
