...
class VanillaMarkov(nn.Module):
    ...
    def get_prev_embeddings(self, token_ids: torch.Tensor) -> torch.Tensor:
        ...
        teacher-forcing producer owns replacing sentinel pads (``-1`` / ``-100`` at
        positions outside the supervision mask) with any valid id before calling,
        mirroring how :func:`~.loss.dspark_loss` sanitizes ``target_ids``. The
        lookup stays strict on purpose: a sentinel that reaches it is a producer
        bug, and clamping here would silently train on row 0 instead of failing.
        """
