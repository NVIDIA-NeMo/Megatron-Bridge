...
"""OmniModal Training Loop for heterogeneous multi-module training.

This module provides the dedicated training loop for OmniModal models with
...
def train_step_omni_modal(
    ...
    """Single OmniModal training step.
    ...
    if losses_reduced:
        ...
        omni_modal_model = unwrap_omni_modal_model(model)
        if omni_modal_model.role is None:
            is_last_stage = True
        elif omni_modal_model.role.has_language_module:
            is_last_stage = omni_modal_model.role.is_last_stage(MIMO_LANGUAGE_MODULE_KEY)
...
def train_omni_modal(
    ...
    Args:
        ...
        global_state: GlobalState containing timers, config, train_state.
        omni_modal_infra: OmniModalInfra with grids, topology, pg_collections.
        multimodule_communicator: MultiModulePipelineCommunicator for P2P.
        checkpoint_manager: CheckpointManager for save operations. Created by
            setup_omni_modal(). If None, a DefaultCheckpointManager is created.
        multimodule_pg_collection: Pre-built PG collection for the pipeline schedule.
            If None, built from omni_modal_infra.
        module_to_grid_tuple: Pre-built (module, grid) pairs for gradient ops.
            If None, built from model and omni_modal_infra.
    """
    timers = global_state.timers
    ...
    # Use pre-built objects from setup_omni_modal if provided, otherwise build them.
    ...
    # Guard against list fallback - OmniModal training requires MultiModuleProcessGroupCollection
    if isinstance(multimodule_pg_collection, list):
        raise RuntimeError(
            "MultiModuleProcessGroupCollection is required for OmniModal training. "
    ...
    # avoid global MPU fallback. In non-colocated OmniModal each rank participates in
    # exactly one module, so "first non-None" unambiguously selects that module's PG.
    active_modules = [(name, pg) for name, pg in omni_modal_infra.pg_collections.items() if pg is not None]
    assert len(active_modules) == 1, (
        f"Non-colocated OmniModal requires exactly one active ProcessGroupCollection per rank, "
        f"got {len(active_modules)}. Colocated OmniModal is not supported by this code path."
    )
    ...
    logger.info(f"Rank {dist.get_rank()}: Starting OmniModal training loop")
    ...
    while train_state.step < train_config.train_iters:
        ...
        loss_dict, skipped_iter, grad_norm, num_zeros_in_grad = train_step_omni_modal(
        ...
        if not cfg.logger.skip_train_metrics_log:
            ...
            # Log iteration-time directly for OmniModal models.
            # training_log only logs this inside a hasattr(config.model, "kv_channels")
            # block which OmniModal models don't satisfy, so we log it here as a workaround.
        ...
        # TODO: OmniModal FLOPs estimation is non-trivial (heterogeneous modules); pass 0 for now.
    ...
    logger.info(f"Rank {dist.get_rank()}: OmniModal training completed")
