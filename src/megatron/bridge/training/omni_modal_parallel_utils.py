...
"""Multi-module process group utilities for OmniModal heterogeneous parallel training.
...
Key functions:
- unwrap_omni_modal_model(): Unwrap Float16Module/DDP to get underlying MimoModel
...
def unwrap_omni_modal_model(model) -> MimoModel:
...
def get_active_module_pg(omni_modal_infra: OmniModalInfra) -> tuple[str, "ProcessGroupCollection"]:
    """Return the (module_name, pg_collection) for the single active module on this rank.

    Non-colocated OmniModal assigns each rank to exactly one module.  This helper
    ...
    """
    active = [(name, pg) for name, pg in omni_modal_infra.pg_collections.items() if pg is not None]
    assert len(active) == 1, (
        f"Non-colocated OmniModal requires exactly one active ProcessGroupCollection per rank, "
        f"got {len(active)}. Colocated OmniModal is not supported by this code path."
    )
    return active[0]
...
def get_module_to_grid_tuple(
    ...
    Args:
        omni_modal_model: The MimoModel instance.
    ...
    unwrapped_model = unwrap_omni_modal_model(omni_modal_model)
