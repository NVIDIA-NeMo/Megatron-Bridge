...
class GPTSFTDataset(Dataset):
    ...
    def __init__(
        ...
        return_padding_mask: Emit a boolean token-validity mask for MoE routing.
            True entries identify padding, independently of the supervision mask.
    ...
    def _build_padding_mask(self, batch: list[dict[str, Any]], max_length: int) -> torch.Tensor:
        """Exclude right padding and synthetic rows while retaining prompt tokens."""
        padding_mask = torch.ones((len(batch), max_length), dtype=torch.bool)
        for index, item in enumerate(batch):
            if not self._is_autogenerated(item):
                # Inputs omit the final token, which is used only as a target.
                input_length = min(max(len(item["input_ids"]) - 1, 0), max_length)
                padding_mask[index, :input_length] = False
        return padding_mask
    ...
    def collate_fn(self, batch):
        ...
        if self.return_padding_mask:
            processed_batch["padding_mask"] = self._build_padding_mask(batch, max_length)

        return processed_batch


class GPTSFTBlendDataset(Dataset):
    """Deterministically blend several compatible GPT SFT datasets.

    Source selection uses a low-discrepancy weighted schedule. Samples within
    each source are shuffled without replacement for every source-local pass
    and repeat only when the requested blend size exceeds that source's size.
    ...
    def __init__(
        self,
        datasets: Sequence[GPTSFTDataset],
        ...
        *,
        ...
        if len(datasets) < 2:
            raise ValueError("A GPT SFT blend requires at least two datasets.")
        if len(datasets) >= np.iinfo(np.int16).max:
            raise ValueError("GPT SFT blends support fewer than 32767 sources.")
        if any(len(dataset) <= 0 for dataset in datasets):
            raise ValueError("GPT SFT blend sources must not be empty.")

        if weights is None:
            normalized_weights = np.asarray([len(dataset) for dataset in datasets], dtype=np.float64)
        else:
            if len(datasets) != len(weights):
                raise ValueError("GPT SFT blend datasets and weights must have the same length.")
            normalized_weights = np.asarray(weights, dtype=np.float64)
            if not np.all(np.isfinite(normalized_weights)) or np.any(normalized_weights <= 0.0):
                raise ValueError("GPT SFT blend weights must be finite and greater than 0.")

        weight_sum = normalized_weights.sum()
        if not np.isfinite(weight_sum) or weight_sum <= 0.0:
            raise ValueError("GPT SFT blend weights must have a positive finite sum.")
        normalized_weights /= weight_sum

        resolved_size = sum(len(dataset) for dataset in datasets) if size is None else size
        if not isinstance(resolved_size, (int, np.integer)) or isinstance(resolved_size, bool) or resolved_size <= 0:
            raise ValueError("GPT SFT blend size must be a positive integer.")
        if not isinstance(seed, int) or isinstance(seed, bool):
            raise ValueError("GPT SFT blend seed must be an integer.")
        ...
        self.pad_seq_length_to_mult = self.datasets[0].pad_seq_length_to_mult
        self.dataset_index, self.dataset_sample_index = self._build_indices()

    def __len__(self) -> int:
        """Return the configured number of blended rows."""
        return self.size

    def __getitem__(self, idx: int) -> dict[str, Any]:
        """Return the row selected by the blend mapping."""
        if idx < -self.size or idx >= self.size:
            raise IndexError(f"GPT SFT blend index {idx} is out of range for size {self.size}.")
        ...
        padding_sample["metadata"] = {**sample.get("metadata", {}), "__AUTOGENERATED__": True}
        if "loss_mask" in sample:
            padding_sample["loss_mask"] = [0] * len(sample["loss_mask"])
        return padding_sample

    def collate_fn(self, batch: list[dict[str, Any]]) -> dict[str, Any]:
        """Collate blended samples with the common source collator."""
