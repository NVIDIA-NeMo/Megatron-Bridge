...
class AssistantMaskBoundaryConfig:
    ...
    Expected input format:
        ...
        ``reasoning`` or ``tool_call`` content. ``part_container_roles``
        controls which top-level roles may be split into nested parts; it
        defaults to assistant-only so tool-call format examples in system/user
        text are not treated as actual tool calls. ``loss_roles`` names
        top-level chat roles that should contribute to loss. ``loss_parts``
        names nested content parts that should contribute to loss.
        ...
        Example::
            ...
            AssistantMaskBoundaryConfig(
                ...
                part_container_roles=("assistant",),
    ...
    Output format:
        ...
        in ``loss_roles`` and part spans whose part name appears in
        ``loss_parts``. Boundary marker tokens are included only when requested
        by the corresponding ``include_*`` fields.
    """
...
def _append_boundary_segment(
    ...
    segments.append((segment_kind, segment_name, _trim_leading_token_ids(ids, start, end, trim_token_ids), end))
...
def _split_boundary_parts(
    ...
    part_container_roles: set[str],
    include_start_tokens_for_parts: set[str],
    include_end_tokens_for_parts: set[str],
    trim_token_ids: Sequence[int],
) -> list[tuple[str, str, int, int]]:
    segments: list[tuple[str, str, int, int]] = []
    if not part_start_tokens or role not in part_container_roles or content_start >= content_end:
        _append_boundary_segment(segments, ids, "role", role, content_start, content_end, trim_token_ids)
        return segments
    ...
    while pos < content_end:
        ...
        if part_start < 0:
            _append_boundary_segment(segments, ids, "role", role, pos, content_end, trim_token_ids)
            break
        if part_start > pos:
            _append_boundary_segment(segments, ids, "role", role, pos, part_start, trim_token_ids)
        ...
        _append_boundary_segment(
            segments, ids, "part", part_role, part_segment_start, part_segment_end, trim_token_ids
        )
...
def _assistant_mask_from_boundary_config(
    ...
    loss_parts = set(boundary_config.loss_parts)
    if not role_start_tokens or not role_end_tokens or (not loss_roles and not loss_parts):
        raise ValueError(
            "AssistantMaskBoundaryConfig must provide non-empty role_start_tokens, role_end_tokens, and at least one "
            "loss role or loss part."
        )
    ...
    part_container_roles = set(boundary_config.part_container_roles)
    ...
    for marker_idx, (start_pos, role, marker_len) in enumerate(deduped_markers):
        ...
        for segment_kind, segment_name, segment_start, segment_end in _split_boundary_parts(
            ids,
            role,
            content_start,
            content_end,
            part_start_tokens,
            part_end_tokens,
            part_container_roles,
            include_start_tokens_for_parts,
            include_end_tokens_for_parts,
            trim_token_ids,
        ):
            if segment_kind == "role":
                should_train_segment = segment_name in loss_roles
            else:
                should_train_segment = segment_name in loss_parts
            if not should_train_segment:
                continue
