...
class AssistantMaskBoundaryConfig:
    ...
    Expected input format:
        ...
        ``input_ids``. ``end_tokens`` is the token-id sequence that terminates a
        role turn. Optional ``part_start_tokens`` and ``part_end_tokens`` define
        nested spans inside a role turn, such as ``reasoning`` or ``tool_call``
        content. ``part_parent_roles`` controls which top-level roles may be
        split into nested parts; it defaults to assistant-only so tool-call
        format examples in system/user text are not treated as actual tool
        calls. ``masked_roles`` names the top-level roles or nested part names
        that should contribute to loss. ``include_*_tokens_for_roles`` and
        ``include_*_tokens_for_parts`` optionally include boundary tokens in the
...
def _assistant_mask_from_boundary_config(
    ...
    end_tokens = _as_token_id_list(boundary_config.end_tokens)
    masked_roles = set(boundary_config.masked_roles)
    if not role_start_tokens or not end_tokens or not masked_roles:
        return None
    ...
    for marker_idx, (start_pos, role, marker_len) in enumerate(deduped_markers):
        ...
        end_start, after_end = find_token_span(ids, end_tokens, content_start)
