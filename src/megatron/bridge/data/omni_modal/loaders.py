...
"""Data loader utilities for OmniModal training."""
...
from megatron.bridge.data.omni_modal.dp_utils import get_omni_modal_sampling_info
...
def build_omni_modal_data_loaders(
    cfg: "ConfigContainer",
    train_state: "TrainState",
    omni_modal_provider: DatasetProvider,
    train_samples: int,
    valid_samples: int,
    test_samples: int,
) -> Tuple[Optional[DataLoader], Optional[DataLoader], Optional[DataLoader]]:
    """Build OmniModal data loaders with globally consistent sampling.
    ...
    ``slice_batch_for_omni_modal`` in the forward step, ensuring consistency with
    ...
    Args:
        ...
        omni_modal_provider: OmniModal dataset provider (e.g., MockOmniModalProvider)
    ...
    Raises:
        ValueError: If cfg.model is not OmniModalProvider or omni_modal_parallelism_config is None.

    Example:
        >>> from megatron.bridge.data.omni_modal import MockOmniModalProvider, build_omni_modal_data_loaders
        >>> provider = MockOmniModalProvider(
        ...     seq_length=2048,
    ...
    if not isinstance(cfg.model, OmniModalProvider):
        raise ValueError("cfg.model must be OmniModalProvider for OmniModal data loading.")

    if cfg.model.omni_modal_parallelism_config is None:
        raise ValueError("omni_modal_parallelism_config must be set for OmniModal data loading.")
    ...
    # slice_batch_for_omni_modal divides the micro-batch contiguously by the module's
    # DP size in forward_step; a non-divisible MBS would leave a remainder.
    micro_batch_size = cfg.train.micro_batch_size
    for mod_name, mod_cfg in cfg.model.omni_modal_parallelism_config.module_parallelisms.items():
        ...
        if micro_batch_size % dp != 0:
            raise ValueError(
                ...
                f"slice_batch_for_omni_modal requires an evenly divisible micro-batch."
            )

    print_rank_0("> building OmniModal train, validation, and test datasets ...")

    # Use cached grids from build_model()
    grids = cfg.model._grids

    sampler_dp_rank, sampler_dp_size, needs_data = get_omni_modal_sampling_info(
        cfg.model.omni_modal_parallelism_config, grids
    )
    ...
    train_ds, valid_ds, test_ds = omni_modal_provider.build_datasets(context)
    ...
    # Per-module DP sub-sharding is done later by slice_batch_for_omni_modal.
    collate_fn = omni_modal_provider.get_collate_fn()
    micro_batch_size = cfg.train.micro_batch_size
    ...
    def _make_loader(dataset, shuffle: bool = True) -> Optional[DataLoader]:
        ...
        return DataLoader(
            ...
            sampler=sampler,
            num_workers=omni_modal_provider.num_workers,
            collate_fn=collate_fn,
            pin_memory=omni_modal_provider.pin_memory,
            drop_last=omni_modal_provider.drop_last,
        )
