...
"""Data parallel utilities for OmniModal data loading."""
...
def get_omni_modal_dp_info(
    omni_modal_cfg: "OmniModalParallelismConfig",
    grids: Dict[str, "HyperCommGrid"],
    ...
    in.  These are used by :func:`slice_batch_for_omni_modal` to sub-shard a global
    micro-batch into per-module DP shards.

    .. note::
        Do **not** use these values to construct a ``DistributedSampler``.
        For sampler construction use :func:`get_omni_modal_sampling_info` instead,
        which returns settings that keep all data-loading ranks synchronised
        on the same sample order.

    Args:
        omni_modal_cfg: OmniModal parallelism configuration.
...
def get_omni_modal_sampling_info(
    omni_modal_cfg: "OmniModalParallelismConfig",
    grids: Dict[str, "HyperCommGrid"],
) -> Tuple[int, int, bool]:
    """Get sampler DP rank, size, and data-loading flag for OmniModal.

    In heterogeneous OmniModal, modules may have different DP sizes.  The data
    loader must give every data-loading rank the **same global micro-batch**
    so that :func:`slice_batch_for_omni_modal` (called in the forward step) can
    ...
    deferred to :func:`slice_batch_for_omni_modal`.
    ...
    # in forward_step via slice_batch_for_omni_modal.
    return 0, 1, needs_data


def slice_batch_for_omni_modal(
    batch: Dict[str, Any],
    dp_rank: int,
    dp_size: int,
) -> Dict[str, Any]:
    ...
    Example:
        ...
        >>> local_batch = slice_batch_for_omni_modal(global_batch, dp_rank=1, dp_size=3)
    ...
    for key, value in batch.items():
        if isinstance(value, torch.Tensor):
            ...
            sliced[key] = slice_batch_for_omni_modal(value, dp_rank, dp_size)
