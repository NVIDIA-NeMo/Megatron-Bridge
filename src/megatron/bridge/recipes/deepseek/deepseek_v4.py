...
def _set_deepseek_v4_optimizer_and_precision(
    ...
    *,
    ...
    if optimizer_type == "adam":
        ...
        if mxfp8:
            # DSv4 Adam MXFP8 trains cleanly, but the default eval path can NaN at
            # the first validation boundary. Keep eval-time FP8 paths in BF16 and
            # avoid MXFP8 param gather/reuse until that eval path is fixed upstream.
...
def _set_deepseek_v4_common_finetune_config(
    cfg: ConfigContainer,
    *,
    hf_path: str,
    peft_enabled: bool,
) -> None:
    """Set common DeepSeek-V4-Flash finetuning defaults."""
    cfg.tokenizer.tokenizer_type = "HuggingFaceTokenizer"
    cfg.tokenizer.tokenizer_model = hf_path
    cfg.tokenizer.vocab_size = None
    ...
    cfg.train.global_batch_size = 32 if peft_enabled else 16
    cfg.train.micro_batch_size = 1
    cfg.train.manual_gc = True
    cfg.train.manual_gc_interval = 5
    cfg.train.manual_gc_eval = 5
...
def _deepseek_v4_flash_finetune_muon_config(
    *,
    peft: str | PEFT | None,
    hf_path: str = "deepseek-ai/DeepSeek-V4-Flash",
) -> ConfigContainer:
    cfg = _peft_common() if peft is not None else _sft_common()
    cfg.model = AutoBridge.from_hf_pretrained(hf_path, trust_remote_code=True).to_megatron_provider(load_weights=False)

    _set_deepseek_v4_common_model_config(cfg, csa_backend="unfused", use_fused_kernels=False)
    _set_deepseek_v4_common_finetune_config(cfg, hf_path=hf_path, peft_enabled=peft is not None)
    _set_deepseek_v4_optimizer_and_precision(cfg, optimizer_type="muon", mxfp8=False)
...
def _deepseek_v4_flash_pretrain_config(
    *,
    ...
    mxfp8: bool = False,
    csa_backend: DSV4_CSA_BACKEND = "unfused",
...
def deepseek_v4_flash_pretrain_config(hf_path: str = "deepseek-ai/DeepSeek-V4-Flash") -> ConfigContainer:
    """Return the DeepSeek-V4-Flash BF16 Adam pre-training config."""
    return _deepseek_v4_flash_pretrain_config(optimizer_type="adam", mxfp8=False, hf_path=hf_path)
...
def deepseek_v4_flash_sft_muon_config(hf_path: str = "deepseek-ai/DeepSeek-V4-Flash") -> ConfigContainer:
    """Return the DeepSeek-V4-Flash full SFT config with Muon BF16."""
    return _deepseek_v4_flash_finetune_muon_config(peft=None, hf_path=hf_path)


def deepseek_v4_flash_peft_muon_config(
    peft: str | PEFT = "lora",
    hf_path: str = "deepseek-ai/DeepSeek-V4-Flash",
) -> ConfigContainer:
    """Return the DeepSeek-V4-Flash LoRA PEFT config with Muon BF16."""
    return _deepseek_v4_flash_finetune_muon_config(peft=peft, hf_path=hf_path)
