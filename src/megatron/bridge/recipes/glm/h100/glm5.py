...
def glm52_753b_pretrain_512gpu_h100_bf16_config() -> ConfigContainer:
    """Return the H100 mock-data pre-training config for GLM-5.2 753B MoE.

    Uses NullTokenizer for mock-data pretraining. DSA requires CP=1 and
    apply_rope_fusion=False (Megatron-Core hard-asserts for the DSA variant).

    Recommended starting parallelism: TP=2, PP=16, EP=32 (512 GPUs).
    """
    cfg = _pretrain_common()

    cfg.model = AutoBridge.from_hf_pretrained(_GLM52_MODEL_ID).to_megatron_provider(load_weights=False)
    ...
    # 78 decoder layers over PP=16: two stages with 4 layers and fourteen
    # stages with 5 layers. Place the single MTP layer before loss.
    cfg.model.pipeline_model_parallel_layout = "Et*4|(t*5|)*14t*4mL"
...
__all__ = [
    "glm52_753b_pretrain_512gpu_h100_bf16_config",
