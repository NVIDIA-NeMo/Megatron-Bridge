...
# Local path while the checkpoint is private. Replace with the published HF id before opening a PR.
NEMOTRON_3_5_NANO_HF_MODEL_ID = (
    "/lustre/fsw/portfolios/coreai/users/liding/public/nano_v35/nemotron35-nano-base-050126"
)
...
def nemotron_3_5_nano_peft_config(
...
def nemotron_3_5_nano_sft_openmathinstruct2_config() -> ConfigContainer:
    ...
    ``nemotron_3_5_nano_sft_config()`` and only overrides ``seq_length`` and
    ``cfg.dataset``.
    """
...
def nemotron_3_5_nano_peft_openmathinstruct2_config(
    peft_scheme: str | PEFT = "lora",
) -> ConfigContainer:
    ...
    ``nemotron_3_5_nano_peft_config()`` and only overrides ``seq_length`` and
    ``cfg.dataset``.
    """
    cfg = nemotron_3_5_nano_peft_config(peft_scheme=peft_scheme)
    seq_length = 4096
    cfg.model.seq_length = seq_length
    cfg.dataset = default_openmathinstruct2_config(seq_length=seq_length, packed_sequence=True)
    return cfg
