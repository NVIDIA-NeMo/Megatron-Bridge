...
"""
OLMoE-7B model recipes with parameterless API.

This module provides pre-configured SFT and PEFT configs for OLMoE-7B model.
...
Note: OLMoE-7B is an MoE (Mixture of Experts) model with 7B total parameters (~1B active).
It uses OlMoEModelProvider instead of AutoBridge.from_hf_pretrained().

Model variants:
- olmoe_7b: 7B total parameters, ~1B active
"""
