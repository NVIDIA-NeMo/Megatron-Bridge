...
class HyperConnectionScaleMapping(MegatronParamMapping):
    ...
    This is the same shape as the DeepSeek-V4 bridge's ``_HCAlphaMapping``; the two should be
    folded into one shared mapping in ``param_mapping.py``.
    """
    ...
    def resolve(self, captures):
        """Resolve the wildcards of all four parameter names against one set of captures."""
    ...
    def hf_to_megatron(self, hf_weights: torch.Tensor, megatron_module: nn.Module) -> torch.Tensor:
        """Import: take element 0 of the packed ``hc_*_scale`` tensor as ``alpha_pre``."""
    ...
    def megatron_to_hf(
        ...
        """Export: repack the three scalar alphas of the owning module into one [3] tensor."""
...
class HyperConnectionScaleSliceMapping(MegatronParamMapping):
    ...
    def resolve(self, captures):
        """Resolve the wildcards of both names and carry the element index through."""
        resolved_megatron, resolved_hf = self._resolve_names(captures)
        return HyperConnectionScaleSliceMapping(resolved_megatron, resolved_hf, self._index)

    def hf_to_megatron(self, hf_weights: torch.Tensor, megatron_module: nn.Module) -> torch.Tensor:
        """Import: take the configured element of the packed ``hc_*_scale`` tensor."""
    ...
    def megatron_to_hf(self, megatron_weights, megatron_module) -> Dict[str, torch.Tensor]:
        """Export: nothing -- the primary mapping already emits all three alphas."""
        return {}
...
class Glm5NextBridge(MegatronModelBridge):
    ...
    ``GPTModel`` built from
    :func:`~megatron.bridge.models.glm.glm5_next_layer_specs.build_glm5_next_layer_spec`.
    ...
    def provider_bridge(self, hf_pretrained: PreTrainedCausalLM) -> Glm5NextModelProvider:
        """Build the provider from the HF config, filling in everything the mapping misses."""
        ...
        provider.linear_attention_freq = [1 if layer_type == "linear_attention" else 0 for layer_type in layer_types]
        ...
        # NoPE MLA + DSA on the full-attention layers. ``experimental_attention_variant`` names
        # the non-linear half of the hybrid; the KDA half comes from the layer spec.
        ...
        indexer_types = [
            indexer_type
            for indexer_type, layer_type in zip(text_config.indexer_types, layer_types)
            if layer_type != "linear_attention"
        ]
        if any(indexer_type != "full" for indexer_type in indexer_types):
            raise NotImplementedError(
                "GLM-5.3-Flash cross-layer DSA index sharing (indexer_types containing 'shared') "
                "is not supported yet: megatron-core's dsa_indexer_topk_freq arithmetic runs over "
                "all layers and would pick KDA layers as index sources in this hybrid."
            )
        ...
        provider.moe_layer_freq = [1 if mlp_type == "sparse" else 0 for mlp_type in text_config.mlp_layer_types]
        ...
        if text_config.hc_eps != _MCORE_MHC_EPS:
            raise ValueError(
                f"hc_eps={text_config.hc_eps} differs from megatron-core's mHC Sinkhorn epsilon {_MCORE_MHC_EPS}."
            )
        return provider

    def mapping_registry(self) -> MegatronMappingRegistry:
        """HF <-> Megatron parameter mappings for the GLM-5.3-Flash language model."""
        ...
        auto_mappings = {
            ...
            # DSA lightning indexer (the k-pool compression parameters have no Megatron
            # counterpart).
        ...
        mappings = [AutoMapping(megatron_param=megatron, hf_param=hf) for megatron, hf in auto_mappings.items()]
