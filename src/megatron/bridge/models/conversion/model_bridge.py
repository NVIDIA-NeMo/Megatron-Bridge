...
class MegatronModelBridge(
    ...
    def load_weights_hf_to_megatron(
        ...
        for task in self._with_progress_tracking(hf_to_megatron_tasks, description):
            ...
            # when no mapping matched that global parameter, which is expected whenever the
            # Megatron model owns a parameter the HF architecture has no counterpart for. It
            # already warned by name, so skip instead of raising an AttributeError that names
            # nothing.
            if task is None:
                continue
    ...
    def stream_weights_hf_to_megatron(
        ...
        for task in conversion_tasks:
            # A None slot means no mapping matched that global parameter (already warned about
            # by build_conversion_tasks); skip it rather than dereferencing None.
            if task is None:
                continue
            # None means megatron module not on current rank, skip if this task is not going to happen
            if task.megatron_module is None:
