...
from typing import TYPE_CHECKING, Dict, Iterable, List, Mapping, Optional, TypeVar, Union
...
class MegatronPeftBridge:
    ...
    @staticmethod
    def normalize_peft_state_dict(peft_sd: Mapping[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        """Strip common PEFT prefixes so keys match Bridge export names (``HFWeightTuple.param_name``).

        Bridge writes tensors under ``base_model.model.<exported_name>``; this helper returns a
        dict keyed by ``<exported_name>`` only.
        """
    ...
    def _merge_fused_fc1_lora_out_tensors(
        self,
        gate: torch.Tensor,
        up: torch.Tensor,
        *,
        ...
        """Invert :meth:`_split_fused_fc1_linear_out_weight` for loading PEFT FC1 LoRA outputs."""

        tp_size = 1 if is_expert else parallel_state.get_tensor_model_parallel_world_size()
        if tp_size <= 1:
            return torch.cat([gate, up], dim=0)

        gate_parts = torch.chunk(gate, tp_size, dim=0)
        up_parts = torch.chunk(up, tp_size, dim=0)
        merged_shards: List[torch.Tensor] = []
        for g_shard, u_shard in zip(gate_parts, up_parts):
            merged_shards.append(torch.cat([g_shard, u_shard], dim=0))
        return torch.cat(merged_shards, dim=0)

    def _scatter_hf_lora_pair_into_megatron_adapter(
        self,
        adapter_task: AdapterWeightConversionTask,
        lora_linear_in_hf: torch.Tensor,
        lora_linear_out_hf: torch.Tensor,
    ) -> None:
        """Scatter a single (lora A, lora B) pair from HF layout into Megatron adapter params."""

        lin = adapter_task.linear_in_task
        lout = adapter_task.linear_out_task
        if lin.megatron_module is None or lin.param_weight is None:
            return

        target_device = lin.param_weight.device
        dtype = lin.param_weight.dtype
        in_t = lora_linear_in_hf.to(device=target_device, dtype=dtype)
        out_t = lora_linear_out_hf.to(device=target_device, dtype=dtype)

        with torch.no_grad():
            conv_in = lin.mapping.hf_to_megatron(in_t, lin.megatron_module)
            if conv_in is not None:
                lin.param_weight.copy_(conv_in)
            if lout.megatron_module is None or lout.param_weight is None:
                return
            conv_out = lout.mapping.hf_to_megatron(out_t, lout.megatron_module)
            if conv_out is not None:
                lout.param_weight.copy_(conv_out)

    def load_peft_adapter_weights_from_state_dict(
        self,
        megatron_model: Union[MegatronModel, List[MegatronModel]],
        peft_sd: Mapping[str, torch.Tensor],
    ) -> None:
        """Load HuggingFace PEFT ``adapter_model.safetensors`` tensors into Megatron adapter modules.

        This reverses weights produced by Bridge's ``convert_adapter_weights_to_peft_state`` /
        ``save_hf_adapter``.  Layout rules mirror :meth:`stream_adapter_weights_megatron_to_hf`.

        Unsupported:
            * Packed grouped-expert exports (raise :class:`NotImplementedError`).
            * GDN fused ``in_proj`` exports (raise :class:`NotImplementedError`).

        Args:
            megatron_model: Megatron stages that already wrap PEFT adapters.
            peft_sd: Raw tensor dict from ``adapter_model.safetensors``.
        """
        ...
        if hasattr(self, "_cached_param_objects_adapter"):
            del self._cached_param_objects_adapter

        norm = self.normalize_peft_state_dict(peft_sd)

        def _peek(hf_suffix: str) -> torch.Tensor:
            if hf_suffix not in norm:
                raise KeyError(
                    f"Missing PEFT tensor {hf_suffix!r} expected for adapter reload. "
                    f"Sample keys (up to 12): {sorted(norm.keys())[:12]}"
                )
            return norm[hf_suffix]
        ...
        for adapter_task in adapter_tasks:
            ...
            if is_grouped_expert:
                expert_linear_out_gathered = self._gather_expert_adapter_weight(linear_out_tensor)
            ...
            packed_expert = False
            base_hf_weight_names_probe: List[str] = []
            if is_grouped_expert:
                base_hf_weight_names_probe = self._get_base_hf_param_names_for_adapter(
                    mapping_registry,
                    adapter_task.global_base_prefix,
                    adapter_task.adapter_key,
                    base_suffixes[0],
                )
                if base_hf_weight_names_probe and not any(
                    self._infer_hf_expert_idx(name) is not None for name in base_hf_weight_names_probe
                ):
                    packed_expert = True
                ...
                if adapter_task.adapter_key is None:
                    ...
                    if per_base_linear_out is not None:
                        if self._is_gdn_in_proj_split(base_hf_weight_names):
                            raise NotImplementedError(
                                "Reloading GDN fused in_proj PEFT adapters from HuggingFace format is not supported."
                            )

                        if self._is_fused_qkv(base_hf_weight_names):
                            out_by_proj: Dict[str, torch.Tensor] = {}
                            for bn, l_b in zip(base_hf_weight_names, linear_out_hf_names):
                                proj_k = self._infer_qkv_projection_from_name(bn)
                                if proj_k is None:
                                    continue
                                out_by_proj[proj_k] = _peek(l_b)
                            merged_out = merge_qkv_weights(
                                config,
                                out_by_proj["q_proj"],
                                out_by_proj["k_proj"],
                                out_by_proj["v_proj"],
                            )
                            lin_a_ref = _peek(linear_in_hf_names[0])
                            if not all(torch.equal(lin_a_ref, _peek(li)) for li in linear_in_hf_names[1:]):
                                raise ValueError("Inconsistent fused QKV LoRA A tensors across HF keys.")
                            self._scatter_hf_lora_pair_into_megatron_adapter(
                                adapter_task,
                                lin_a_ref,
                                merged_out,
                            )
                        ...
                        if self._is_fused_fc1_gate_up(base_hf_weight_names, _peek(linear_out_hf_names[0])):
                            gate_name = up_name = None
                            for bn, lb in zip(base_hf_weight_names, linear_out_hf_names):
                                if self._is_fused_fc1_gate_proj(bn):
                                    gate_name = lb
                                elif self._is_fused_fc1_up_proj(bn):
                                    up_name = lb
                            if gate_name is None or up_name is None:
                                raise ValueError("Fused FC1 adapter export expects gate/up LoRA tensors.")
                            gate_tensor = _peek(gate_name)
                            up_tensor = _peek(up_name)
                            merged_out = self._merge_fused_fc1_lora_out_tensors(
                                gate_tensor,
                                up_tensor,
                                is_expert=is_expert_linear(adapter_task.global_base_prefix),
                            )
                            lin_a_ref = _peek(linear_in_hf_names[0])
                            self._scatter_hf_lora_pair_into_megatron_adapter(adapter_task, lin_a_ref, merged_out)
                            continue

                        for idx, base_name in enumerate(base_hf_weight_names):
                            cur_hf_out = per_base_linear_out.get(base_name)
                            if isinstance(cur_hf_out, _AbsentProjectionSentinel):
                                continue
                            self._scatter_hf_lora_pair_into_megatron_adapter(
                                adapter_task,
                                _peek(linear_in_hf_names[idx]),
                                _peek(linear_out_hf_names[idx]),
                            )
                        continue

                self._scatter_hf_lora_pair_into_megatron_adapter(
                    adapter_task,
                    _peek(linear_in_hf_names[0]),
                    _peek(linear_out_hf_names[0]),
                )

    def load_peft_adapter_weights_from_hf_dir(
        self,
        megatron_model: Union[MegatronModel, List[MegatronModel]],
        adapter_dir: Union[str, os.PathLike[str]],
    ) -> None:
        """Load ``adapter_model.safetensors`` from a HuggingFace PEFT directory."""
        ...
        self.load_peft_adapter_weights_from_state_dict(megatron_model, load_file(str(weights_path)))
