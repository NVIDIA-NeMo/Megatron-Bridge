...
parameters (specifically amax values) during HuggingFace <-> Megatron conversions.
"""
...
class MoeAmaxFanoutMapping(AmaxMapping):
    ...
    def megatron_to_hf(self, megatron_weights, megatron_module):
        ...
        Note:
            The megatron_module here is the module that owns the quantizer parameter,
            which may be a low-level module like a Linear layer or the quantizer itself.
            However, during task building in model_bridge.py, the model's TransformerConfig
            is attached to modules that don't have a config attribute (see lines 1011-1014
            in model_bridge.py). This ensures megatron_module.config.num_moe_experts is
            available for MoE models.
        """
        base = super().megatron_to_hf(megatron_weights, megatron_module)
        if not base:
            return {}
        ...
        if num_experts is None:
            ...
            if hasattr(megatron_module, 'config') and hasattr(megatron_module.config, 'num_moe_experts'):
                ...
                # Some configs use 'num_experts' instead of 'num_moe_experts'
                num_experts = megatron_module.config.num_experts
            else:
                # Fallback: try to infer from the module structure (less reliable)
    ...
    def _infer_num_experts(self, megatron_module):
        ...
        # Walk up to find a module with num_experts info
    ...
    for mapping in mappings:
        ...
        if is_moe_expert_weight:
            ...
            if debug_quant_mapping:
                print(f"[DEBUG] MoE amax mapping: {megatron_param} -> {new_megatron_param}")
            ...
            if isinstance(mapping.hf_param, dict):
                ...
                if hf_patterns:
                    # print('adding MoeAmaxFanoutMapping 1:', new_megatron_param, hf_patterns)
                ...
                if '.weight' in mapping.hf_param:
                    # print('adding MoeAmaxFanoutMapping 2:', new_megatron_param, mapping.hf_param)
        ...
        if debug_quant_mapping:
            print(f"[DEBUG] Regular amax mapping: {megatron_param} -> {new_megatron_param}")
