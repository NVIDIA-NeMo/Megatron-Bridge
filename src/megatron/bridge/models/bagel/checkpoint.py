...
"""Strict Bridge adapter from a native BAGEL checkpoint to MCore.
...
_ACTIVE_TARGETS: ContextVar[dict[int, str] | None] = ContextVar("bagel_native_checkpoint_targets", default=None)
...
class _TensorSource:
    ...
    def require_exact_keys(self, expected: set[str]) -> None:
        ...
        if missing or unexpected:
            ...
            raise KeyError("Native BAGEL checkpoint tensor set differs from the model: " + "; ".join(details))
...
class _LLMConfig(Protocol):
    """BAGEL language configuration fields required by the remapper."""
...
def _fuse_qkv(
    ...
    *,
    ...
    if num_query_groups <= 0 or num_attention_heads % num_query_groups != 0:
        raise ValueError(
            f"num_attention_heads={num_attention_heads} must be divisible by num_query_groups={num_query_groups}"
        )
    if head_dim <= 0:
    ...
    if query.shape[1:] != key.shape[1:] or query.shape[1:] != value.shape[1:]:
        raise ValueError(
            f"Q, K, and V trailing shapes differ: {tuple(query.shape)}, {tuple(key.shape)}, {tuple(value.shape)}"
        )
...
def _unfuse_qkv(
    ...
    key = grouped[:, queries_per_group : queries_per_group + 1].reshape(num_query_groups * head_dim, *trailing_shape)
    value = grouped[:, queries_per_group + 1 :].reshape(num_query_groups * head_dim, *trailing_shape)
...
def _fuse_glu(gate: torch.Tensor, up: torch.Tensor) -> torch.Tensor:
    ...
    if gate.shape != up.shape:
        raise ValueError(f"GLU gate/up shapes differ: gate={tuple(gate.shape)}, up={tuple(up.shape)}")
    if gate.ndim == 0:
...
def _unfuse_glu(fused: torch.Tensor, *, ffn_hidden_size: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    if fused.shape[0] != 2 * ffn_hidden_size:
        raise ValueError(f"Fused GLU leading dimension is {fused.shape[0]}, expected {2 * ffn_hidden_size}")
...
def _copy_parameter(destination: torch.Tensor, source: torch.Tensor, name: str) -> None:
    ...
    if active_targets is not None:
        ...
        if previous_name != name:
            raise ValueError(f"MCore destination was initialized twice: {previous_name!r} and {name!r}")
...
def _copy_attention_branch(
    ...
    *,
    ...
    for branch_name, actual, source in zip(("query", "key", "value"), remapped_weight, (query, key, value)):
        _assert_equal_to_source(actual, source, f"{qkv_prefix} inverse-remapped {branch_name} weight")
...
def _copy_mlp_branch(reader: _TensorSource, mcore_mlp, source_prefix: str) -> None:
    ...
    actual_gate, actual_up = _unfuse_glu(mcore_mlp.linear_fc1.weight.detach(), ffn_hidden_size=gate.shape[0])
...
def _validate_unsharded_model(language_model) -> None:
    for stage_flag in ("pre_process", "post_process"):
        if hasattr(language_model, stage_flag) and not getattr(language_model, stage_flag):
            raise NotImplementedError(f"Native BAGEL initialization requires a complete model with {stage_flag}=True")
    ...
    for name in ("tp", "pp"):
        ...
        if size != 1:
            raise NotImplementedError(f"Native BAGEL initialization requires TP=1 and PP=1; {name.upper()}={size}")
...
def _metadata_int(metadata: Mapping[str, str], key: str) -> int:
    ...
    try:
        ...
        raise ValueError(f"Native BAGEL checkpoint metadata {key}={value!r} is not an integer") from error
...
def _validate_metadata(
    ...
    if actual_format != _FORMAT_VERSION:
        raise ValueError(
            f"Unsupported native BAGEL checkpoint format_version={actual_format}; expected {_FORMAT_VERSION}"
        )
    for key, expected in (("model_seed", expected_model_seed), ("world_size", expected_world_size)):
        actual = _metadata_int(metadata, key)
        if actual != expected:
            raise ValueError(f"Native BAGEL checkpoint metadata {key}={actual}; expected {expected}")


def _initialize_language(reader: _TensorSource, language_model, llm_config: _LLMConfig) -> None:
...
def _assert_exact_target_coverage(mimo_model: torch.nn.Module, initialized_targets: Mapping[int, str]) -> None:
    ...
    latent_position = mimo_model.modality_submodules["diffusion"].encoders["latent_position_ids"].pos_embed
    expected_targets.setdefault(
        id(latent_position), "modality_submodules.diffusion.encoders.latent_position_ids.pos_embed"
    )

    missing = sorted(name for target_id, name in expected_targets.items() if target_id not in initialized_targets)
    unexpected = sorted(name for target_id, name in initialized_targets.items() if target_id not in expected_targets)
    if missing or unexpected:
        details = []
        if missing:
            details.append(f"missing={missing}")
...
def initialize_bagel_from_native_checkpoint(
    ...
    *,
    ...
    The checkpoint must contain the complete native ``Bagel.state_dict()``.
    Exported initialization checkpoints also validate ``format_version``,
    ``model_seed``, and ``world_size`` metadata. Official BAGEL EMA releases
    omit that metadata and may set ``validate_metadata=False``. Every tensor
    must map exactly once; missing and additional tensors are rejected before
    any parameter is modified.
    """
    ...
    with safe_open(checkpoint_path, framework="pt", device="cpu") as checkpoint:
        ...
        expected_keys = _language_keys(int(llm_config.num_hidden_layers)) | vision_keys | set(_AUXILIARY_KEYS)
