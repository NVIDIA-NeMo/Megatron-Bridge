...
class SarvamMoEModelProvider(GPTModelProvider):
    """Base model provider for Sarvam MoE Models."""

    transformer_layer_spec: Union["ModuleSpec", Callable[["GPTModelProvider"], "ModuleSpec"]] = partial(
        get_gpt_decoder_block_spec, use_transformer_engine=HAVE_TE, normalization="RMSNorm", vp_stage=None
    )
    ...
    recompute_modules: List[str] = field(default_factory=lambda: ["layernorm", "shared_experts", "moe", "mlp", "moe_act"])
    

@dataclass
class SarvamMoEModelProvider30B(GPTModelProvider):
    """ Config for Sarvam MoE 30B """
