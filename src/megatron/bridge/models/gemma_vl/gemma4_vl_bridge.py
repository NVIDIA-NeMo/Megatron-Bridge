...
"""
Megatron Bridge for Gemma 4 (CausalLM text-only and ConditionalGeneration VL).

Supports all Gemma 4 variants:
  - MoE (``enable_moe_block=True``): ``Gemma4ForCausalLM`` / ``Gemma4ForConditionalGeneration``
  - Dense (``enable_moe_block=False``): same HF classes, dispatched via ``Gemma4DenseProvider``
...
Bridge conversion architecture:
  AutoBridge.from_hf_pretrained("google/gemma-4-E4B-it")
    └─ Gemma4VLBridge (registered for Gemma4ForConditionalGeneration)
         ...
         └─ _dense_mapping_registry() / _moe_vl_mapping_registry()

  AutoBridge.from_hf_pretrained("google/gemma-4-26B-A4B")
    └─ Gemma4Bridge (registered for Gemma4ForCausalLM, MoE or Dense)
"""
