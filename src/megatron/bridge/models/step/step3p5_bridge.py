...
from megatron.bridge.models.step.modeling_step3p5 import Step3p5ForCausalLM
...
AutoModelForCausalLM.register(Step3p5Config, Step3p5ForCausalLM, exist_ok=True)
...
class Step3p5Bridge(MegatronModelBridge):
    ...
    def provider_bridge(self, hf_pretrained: PreTrainedCausalLM) -> GPTModelProvider:
        ...
        # rope_theta is a per-layer list (5_000_000 for global-attn layers, 10_000 for SWA layers).
        # MCore uses a single global rotary_base; use the global-attention value (max).
        # TODO(shifang): change to use per-layer rotary_base
        rope_theta = hf_config.rope_theta
        if isinstance(rope_theta, list):
            provider.rotary_base = max(rope_theta)
