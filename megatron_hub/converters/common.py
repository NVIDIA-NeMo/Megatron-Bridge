...
def get_full_mcore_state_dict(dist_ckpt_folder: Path, model_cfg: Any) -> dict[str, torch.Tensor]:
    """Load a full, unsharded state dict from a megatron_hub distributed checkpoint.
    ...
    Args:
        dist_ckpt_folder: Path to the megatron_hub distributed checkpoint directory
...
class BaseImporter(ABC):
    ...
    Args:
        ...
        output_path: Path to the directory where the megatron_hub checkpoint will be saved.
    """
    ...
    def tokenizer(self) -> "_HuggingFaceTokenizer":
        """Get a megatron_hub _HuggingFaceTokenizer wrapper for the source HF model.

        Downloads/copies the tokenizer assets and initializes the wrapper.

        Returns:
            A megatron_hub _HuggingFaceTokenizer instance.
        """
...
class BaseExporter(ABC):
    """Abstract Base Class for exporting models from megatron_hub Tron format to Hugging Face.
    ...
    Args:
        input_path: Path to the input megatron_hub Tron checkpoint directory.
    ...
    @abstractmethod
    def hf_config(self) -> "PretrainedConfig":
        """Get the Hugging Face configuration object derived from the megatron_hub config.
    ...
    @property
    def tron_config(self) -> GPTConfig | T5Config:
        """Get the megatron_hub Tron configuration loaded from the checkpoint.

        Returns:
            The loaded megatron_hub Tron configuration instance (e.g., GPTConfig).
    ...
    def tokenizer(self) -> Optional["AutoTokenizer"]:
        ...
        or attempts to load one found within the megatron_hub checkpoint during `apply`.
    ...
    @abstractmethod
    def convert_state(self, source: Any, target: Any) -> None:
        """Convert the state dict from the source megatron_hub model to the target HF model.

        Must be implemented by subclasses.

        Args:
            source: A helper object (_ModelState) containing the loaded megatron_hub state dict.
    ...
    def init_tron_model(self) -> tuple[dict[str, torch.Tensor], GPTConfig | T5Config]:
        """Load the megatron_hub Tron model state dict and config from a distributed checkpoint.

        Loads the full state dict directly without initializing the full megatron_hub model
        for memory efficiency.

        Returns:
            A tuple containing:
            - The loaded, unsharded megatron_hub state dict.
            - The loaded megatron_hub model configuration (e.g., GPTConfig).
        """
        ...
        if self._hf_tokenizer_path is None:
            # Try to build tokenizer from the megatron_hub checkpoint
            ...
            if (
                ...
                logger.warning("Failed to find Huggingface tokenizer in the megatron_hub checkpoint")
    ...
    def apply(self) -> Path:
        """Run the full conversion process from megatron_hub Tron to Hugging Face.

        Loads the megatron_hub state dict and config, initializes the target HF model,
