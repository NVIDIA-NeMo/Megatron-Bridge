...
| Text SFT or PEFT from reusable prepared data | Recommended for materialized JSONL, offline packed Parquet, source blending, or finite epochs | [Prepared text-only](text-only-sft/README.md) | `GPTSFTDatasetConfig` → `GPTSFTDatasetBuilder` |
...
- Choose the prepared [text-only SFT](text-only-sft/README.md) path when you need reusable local JSONL, finite `num_epochs`, offline packed Parquet, or a weighted blend of packed Parquet sources.
