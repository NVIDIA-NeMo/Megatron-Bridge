...
"""
Kimi-K2 (1T) Pretraining Script with YAML and CLI Configuration Overrides.

This script provides a flexible way to pretrain Kimi-K2 models using Megatron-Bridge with support for
both YAML configuration files and command-line overrides using Hydra-style syntax.

Examples:
    Basic usage with default configuration:
        $ python pretrain_kimi_k2.py

    Using a custom YAML config file:
        $ python pretrain_kimi_k2.py --config-file my_custom_config.yaml

    Using CLI overrides only:
        $ python pretrain_kimi_k2.py model.tensor_model_parallel_size=4 train.train_iters=100000

    Combining YAML and CLI overrides (CLI takes precedence):
        $ python pretrain_kimi_k2.py --config-file conf/my_config.yaml \
...
# Assumes this script (pretrain_kimi_k2.py) is in Megatron-Bridge/examples/recipes/kimi/
...
def parse_cli_args() -> Tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(
        description="Pretrain Kimi-K2 (1T) model using Megatron-Bridge with YAML and CLI overrides",
...
def main() -> None:
    """
    Entry point for the Kimi-K2 (1T) pretraining script.
    ...
    Examples of CLI usage:
        ...
        python pretrain_kimi_k2.py optimizer.lr=0.0003

        # Custom config file with additional overrides
        python pretrain_kimi_k2.py --config-file my_config.yaml train.train_iters=500000

        # Multiple overrides for distributed training
        python pretrain_kimi_k2.py \
            model.tensor_model_parallel_size=2 \
            model.pipeline_model_parallel_size=16 \
            train.global_batch_size=4096
    """
    args, cli_overrides = parse_cli_args()

    logger.info("Megatron-Bridge Kimi-K2 (1T) Pretraining Script with YAML & CLI Overrides")
    logger.info("------------------------------------------------------------------")
