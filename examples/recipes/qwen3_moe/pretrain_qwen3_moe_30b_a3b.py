...
"""
Qwen3 MoE (30B-A3B) Pretraining Script with YAML and CLI Configuration Overrides.

This script provides a flexible way to pretrain Qwen3 MoE 30B-A3B models using Megatron-Bridge
with support for both YAML configuration files and command-line overrides using Hydra-style syntax.

Examples:
    Basic usage with default configuration:
        $ torchrun --nproc_per_node=8 examples/recipes/qwen3_moe/pretrain_qwen3_moe_30b_a3b.py
    ...
    Combining YAML and CLI overrides (CLI takes precedence):
        $ torchrun --nproc_per_node=8 examples/recipes/qwen3_moe/pretrain_qwen3_moe_30b_a3b.py \\
            --config-file conf/qwen3_moe_30b_a3b_pretrain_override_example.yaml \\
            model.pipeline_dtype=torch.bfloat16 \\
            train.global_batch_size=256

Notes:
    - To run other Qwen3 MoE variants (e.g., 235B-A22B), use the corresponding recipe
      in `megatron.bridge.recipes.qwen.qwen3_moe` or create a similar script importing
...
from megatron.bridge.recipes.qwen.qwen3_moe import qwen3_30b_a3b_pretrain_config as pretrain_config
...
# Assumes this script is in Megatron-Bridge/examples/recipes/qwen3_moe/
# and the config is in a 'conf' subdirectory.
SCRIPT_DIR: Path = Path(__file__).parent.resolve()
DEFAULT_CONFIG_FILENAME: str = "qwen3_moe_30b_a3b_pretrain_override_example.yaml"
...
def parse_cli_args() -> Tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(
        description="Pretrain Qwen3 MoE 30B-A3B model with YAML and CLI overrides",
    ...
    parser.add_argument(
        ...
        help="Path to the YAML OmegaConf override file. Default: conf/qwen3_moe_30b_a3b_pretrain_override_example.yaml",
    )
...
def main() -> None:
    """
    Entry point for the Qwen3 MoE 30B-A3B pretraining script.
    ...
    logger.info("Megatron-Bridge Qwen3 MoE 30B-A3B Pretraining Script with YAML & CLI Overrides")
    logger.info("-------------------------------------------------------------------------------")
