...
HF_MODEL_ID="${HF_MODEL_ID:-MiniMaxAI/MiniMax-M2}"
# TP*PP*EP must equal NODES*GPUS_PER_NODE for these data-parallel-free layouts.
# EP must divide 256 (the number of experts).
...
for config in "${PARALLELISM_CONFIGS[@]}"; do
    ...
    echo "Submitting MiniMax-M2 roundtrip: TP=${TP}, PP=${PP}, EP=${EP}"
    "${CONVERT_SH}" roundtrip \
        ...
        "${MOUNT_ARGS[@]}" \
        "${ENV_ARGS[@]}" \
        --experiment-name "minimax-m2-roundtrip-tp${TP}-pp${PP}-ep${EP}" \
