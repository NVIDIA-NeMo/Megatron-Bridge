...
Run via wrapper:
    bash examples/models/omni_modal/run_omni_modal_checkpoint_resume.sh
Or manually:
    CKPT_DIR=$(mktemp -d)
    torchrun --nproc_per_node=8 examples/models/omni_modal/omni_modal_checkpoint_resume_e2e.py --phase save   --ckpt-dir $CKPT_DIR
    torchrun --nproc_per_node=8 examples/models/omni_modal/omni_modal_checkpoint_resume_e2e.py --phase resume --ckpt-dir $CKPT_DIR
"""
...
# ---------------------------------------------------------------------------
# Model helpers (same as omni_modal_training_e2e.py)
# ---------------------------------------------------------------------------
...
def _build_data_iterators(cfg, omni_modal_infra, *, train_state=None):
...
def _build_config(
    ...
    mock_data_provider: MockOmniModalProvider,
    ...
    *,
    ...
    par_cfg = omni_modal_provider.omni_modal_parallelism_config
...
def _run_phase_save(ckpt_dir: str) -> None:
    ...
    if not hasattr(omni_modal_provider, "fp8"):
        omni_modal_provider.fp8 = None
...
def main():
    """Entry point for the OmniModal checkpoint-resume end-to-end example."""
    global _rank_log_file
    ...
    log_dir = "/tmp/omni_modal_resume_e2e_logs"
