...
Exercises the full training loop: pretrain_omni_modal -> setup_omni_modal -> train_omni_modal
on 8 GPUs with synthetic data using the real data pipeline.
LLM on ranks 0-3 (TP=4), vision encoder on ranks 4-7 (TP=4).

Run:
    torchrun --nproc_per_node=8 examples/models/omni_modal/omni_modal_training_e2e.py
"""
...
def _build_mock_data_provider() -> MockOmniModalProvider:
    """Build a MockOmniModalProvider with HF processor (CLIP) and tokenizer (GPT-2)."""
...
def _build_data_iterators(cfg, omni_modal_infra):
    ...
    train_loader, valid_loader, _ = build_omni_modal_data_loaders(
        cfg=cfg,
...
def main():
    """Entry point for the OmniModal training end-to-end example."""
    global _rank_log_file
    ...
    log_dir = "/tmp/omni_modal_e2e_logs"
