...
def _build_hf_data_provider(dataset_root: str) -> HFOmniModalDatasetProvider:
    """Build an HFOmniModalDatasetProvider for liuhaotian/LLaVA-Pretrain."""
    provider = HFOmniModalDatasetProvider(
...
def _build_data_iterators(cfg, _omni_modal_infra, *, train_state=None):
    """Build data iterators compatible with setup_omni_modal's build_data_iterators_fn.

    Signature: (cfg, omni_modal_infra, *, train_state=None) -> (train_iter, valid_iter)
    Uses build_omni_modal_data_loaders which auto-detects MIMO path via cfg.model.
    ...
    train_loader, _, _ = build_omni_modal_data_loaders(
        cfg=cfg,
...
def _build_config(
    omni_modal_provider: OmniModalProvider,
    data_provider: HFOmniModalDatasetProvider,
    ...
    # per-module DP sub-sharding is handled by slice_batch_for_omni_modal in the
...
from megatron.bridge.training.omni_modal_step import forward_step as omni_modal_forward_step
...
def parse_args():
    """Parse command-line arguments for the OmniModal LLaVA training example."""
    parser = argparse.ArgumentParser(description="OmniModal LLaVA training")
...
def main():
    """Entry point for the OmniModal LLaVA training example."""
    global _rank_log_file
    ...
    # NOTE: _set_omni_modal_random_seeds() in setup_omni_modal re-seeds all RNGs with
    ...
    log_dir = os.environ.get("OMNI_MODAL_LOG_DIR", "/tmp/omni_modal_llava_logs")
    ...
    omni_modal_parallelism_config = _build_parallelism_config()
    ...
    llm_pp_size = omni_modal_parallelism_config.module_parallelisms["language"].pipeline_model_parallel_size
    ...
    if args.language_model_checkpoint or args.vision_encoder_checkpoint:
        omni_modal_provider.register_pre_wrap_hook(
    ...
    if not hasattr(omni_modal_provider, "num_moe_experts"):
        omni_modal_provider.num_moe_experts = None
    ...
    # 7. Run training
    _log("launching pretrain_omni_modal")
    pretrain_omni_modal(
        cfg=cfg,
        forward_step_func=omni_modal_forward_step,
