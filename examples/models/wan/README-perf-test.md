...
This guide provides concise steps to set up the environment and run WAN pretraining and inference. It uses the current Megatron-Bridge `main` branch, pins the other repository commits, and shows explicit commands for the 1.3B and 14B configurations.
...
Set up Megatron-Bridge from `main`, pin DFM and Megatron-LM, and install the other dependencies.
...
# Megatron-Bridge (current main includes the integrity-locked WAN installer)
rm -rf /opt/Megatron-Bridge
git clone --no-checkout https://github.com/NVIDIA-NeMo/Megatron-Bridge.git
git -C Megatron-Bridge checkout main
