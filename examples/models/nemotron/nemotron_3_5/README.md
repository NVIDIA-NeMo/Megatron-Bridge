...
See the SLURM scripts in [nano/](nano/): [slurm_pretrain.sh.no_share](nano/slurm_pretrain.sh.no_share), [slurm_sft.sh.no_share](nano/slurm_sft.sh.no_share), [slurm_peft.sh.no_share](nano/slurm_peft.sh.no_share).
...
```bash
export NEMO_HOME=${WORKSPACE}/cache/nemo  # shared FS, same value the slurm scripts use
...
uv run python -m torch.distributed.run --nproc_per_node=8 \
  examples/peft/merge_lora.py \
    ...
    --hf-model-path nvidia/Nemotron-3.5-Nano-Base \
