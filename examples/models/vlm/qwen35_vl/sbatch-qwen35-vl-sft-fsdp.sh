...
# =============================================================================
# Qwen3.5-VL 35B-A3B SFT with Megatron FSDP
#
# This script launches Qwen3.5-VL SFT training with Megatron FSDP enabled.
# FSDP provides efficient memory usage via all-gather / reduce-scatter overlap.
#
# Prerequisites:
#   1. Convert HF checkpoint to mcore fsdp_dtensor format:
#        bash download-convert-qwen35-vl.sh 35B-A3B
#   2. Set WORKSPACE, CONTAINER_IMAGE, and WANDB_API_KEY below.
#
# Usage:
#   sbatch sbatch-qwen35-vl-sft-fsdp.sh
# =============================================================================

#SBATCH -A <your_account>
#SBATCH -p <your_partition>
#SBATCH -J megatron-bridge.qwen35vl-sft-fsdp
#SBATCH -t 00:20:00
#SBATCH -N8
...
# ---- Parallelism -----------------------------------------------------------
#TP=1; PP=1; CP=1; EP=8
TP=1; PP=1; CP=1; EP=2; NUM_DIST_OPT_INSTANCES=1
...
DDP_NCCL_UB="false"                  # disabled: MoE+VLM FSDP units all fallback, UB causes hang
DDP_FSDP_DB_PERSIST_BUF="true"       # persistent buffer for non-fitting FSDP units (fixes storage-of-size-0 crash)
...
MOE_SHARED_EXPERT_OVERLAP="false"    # overlap shared expert with EP comm (requires alltoall dispatcher)

# ---- Performance / Fusions --------------------------------------------------
GRAD_REDUCE_IN_FP32="true"           # gradient reduce in fp32 precision
GRAD_ACCUM_FUSION="true"             # fused gradient accumulation (needs fused_weight_gradient_mlp_cuda) TE>=2.10 for FSDP
...
OVERLAP_OVERRIDES="\
    ...
    ddp.fsdp_db_use_persist_buf_on_alloc_fail=$DDP_FSDP_DB_PERSIST_BUF \
    ddp.pad_buckets_for_high_nccl_busbw=$DDP_PAD_BUCKETS \
    ddp.num_distributed_optimizer_instances=$NUM_DIST_OPT_INSTANCES \
