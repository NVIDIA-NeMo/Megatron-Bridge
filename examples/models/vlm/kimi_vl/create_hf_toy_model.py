...
def _is_quantized_expert_key(key: str) -> bool:
    """Check if a state dict key corresponds to an INT4-quantized expert weight."""
...
def _maybe_quantize_state_dict(model, config) -> dict | None:
    """If the source config has a quantization_config, pack expert weights to INT4.

    Uses the same quantize_to_int4 utility as the Megatron bridge export path,
    replacing each quantizable expert weight with (weight_packed, weight_scale,
    weight_shape) in the state dict.

    Returns:
        Modified state dict with INT4 packed weights, or None if no quantization needed.
    """
    ...
    keys_to_quantize = [k for k in state_dict if _is_quantized_expert_key(k)]
    if not keys_to_quantize:
        return None
    ...
    for key in keys_to_quantize:
        ...
        base = key[: -len(".weight")]
        state_dict[f"{base}.weight_packed"] = packed
        state_dict[f"{base}.weight_scale"] = scale
        state_dict[f"{base}.weight_shape"] = shape

    print(f"  Quantized {len(keys_to_quantize)} expert weights to INT4")
    return state_dict
...
def main() -> None:
    ...
    # Remove quantization_config before from_config (it can't handle it),
    # but save it so we can apply quantization afterwards.
    quant_config_backup = {}
    for cfg in (config, getattr(config, "text_config", None)):
        if cfg is not None and hasattr(cfg, "quantization_config"):
            quant_config_backup[id(cfg)] = cfg.quantization_config
            delattr(cfg, "quantization_config")

    model = AutoModelForCausalLM.from_config(config, trust_remote_code=trust_remote_code)
    model = model.bfloat16()

    # Restore quantization_config and apply real quantization.
    for cfg in (config, getattr(config, "text_config", None)):
        if cfg is not None and id(cfg) in quant_config_backup:
            cfg.quantization_config = quant_config_backup[id(cfg)]
