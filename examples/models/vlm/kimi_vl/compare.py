...
def _load_hf_model(args, is_vl_model: bool):
    ...
    # Check if model uses compressed-tensors quantization (e.g. INT4 packed experts).
    # compressed_tensors has a bug with meta-device biases in VL models, so we bypass
    # it by removing the quantization config, loading the skeleton, then manually
    # dequantizing and assigning the packed INT4 weights.
    hf_config = AutoConfig.from_pretrained(args.hf_model_path, trust_remote_code=trust_remote)
    quant_cfg = getattr(hf_config, "quantization_config", None)
    is_compressed_tensors = isinstance(quant_cfg, dict) and quant_cfg.get("quant_method") == "compressed-tensors"
    ...
    if is_compressed_tensors:
        ...
        print_rank_0("Detected compressed-tensors quantization, loading with manual dequantization...")
        # Strip quantization config so from_pretrained skips compressed_tensors
        ...
        # Load safetensors state dict and dequantize INT4 packed weights
        import glob
        import json

        index_file = os.path.join(args.hf_model_path, "model.safetensors.index.json")
        if os.path.exists(index_file):
            with open(index_file) as f:
                shard_files = set(json.load(f)["weight_map"].values())
        else:
            shard_files = [os.path.basename(p) for p in glob.glob(os.path.join(args.hf_model_path, "*.safetensors"))]

        raw_state = {}
        for shard in shard_files:
            with safe_open(os.path.join(args.hf_model_path, shard), framework="pt", device="cpu") as f:
                for key in f.keys():
                    raw_state[key] = f.get_tensor(key)

        # Find packed INT4 weights and dequantize them
        assigned = set()
        packed_bases = set()
        for key in raw_state:
            if key.endswith(".weight_packed"):
                packed_bases.add(key[: -len(".weight_packed")])
        for base in packed_bases:
            ...
            setattr(module, parts[-1], torch.nn.Parameter(weight.to(torch.bfloat16)))
            assigned.add(param_key)

        print_rank_0(f"Dequantized {len(packed_bases)} and assigned {len(assigned)} INT4 parameter groups")
