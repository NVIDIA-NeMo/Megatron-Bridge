...
# Default parallelism: TP=2, ETP=1, EP=8, CP=1, MBS=2, GBS=64, packed sequences,
...
#   TP=4 ETP=1 EP=4 CP=1 PACKED_SEQ=false sbatch slurm_peft_valor32k_avqa.sh
#
# Usage:
...
echo "Parallelism: TP=$TP ETP=$ETP EP=$EP CP=$CP (packed=$PACKED_SEQ, temporal video tpd=2, LoRA)"
echo "======================================"

RUN_TAG=tp${TP}etp${ETP}ep${EP}cp${CP}pack${PACKED_SEQ}_iter${TRAIN_ITERS}
OUTPUT_DIR=${WORKSPACE}/results/${RECIPE}_lora_${RUN_TAG}
...
CLI_OVERRIDES="\
    ...
    logger.wandb_exp_name=${RECIPE}_lora_${RUN_TAG}"
