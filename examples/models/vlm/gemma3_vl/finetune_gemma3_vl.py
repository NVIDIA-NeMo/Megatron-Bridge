...
"""
Gemma3-VL Finetuning Script with YAML and CLI Configuration Overrides.

This mirrors the Llama example flow and uses the Gemma3-VL recipe helpers.
You can pick a specific recipe via `--recipe`, e.g., `gemma3_vl_finetune_config`.
...
Examples:
    Loading pretrained weights (recommended for finetune):
        ...
        2) Run finetune using the imported checkpoint:
           $ torchrun --nproc_per_node=8 examples/models/vlm/gemma3_vl/finetune_gemma3_vl.py \
               --pretrained-checkpoint /path/to/megatron_ckpt

    Using a custom YAML config file:
        $ torchrun --nproc_per_node=8 finetune_gemma3_vl.py --config-file conf/gemma3_vl_pretrain_override_example.yaml

    CLI overrides:
        $ torchrun --nproc_per_node=8 finetune_gemma3_vl.py model.tensor_model_parallel_size=4 train.train_iters=100000

    Selecting a specific recipe:
        $ torchrun --nproc_per_node=8 finetune_gemma3_vl.py --recipe gemma3_vl_4b_finetune_config
"""
...
from megatron.bridge.recipes.gemma3_vl import gemma3_vl as gemma3_vl_recipes
...
DEFAULT_CONFIG_FILENAME: str = "gemma3_vl_pretrain_override_example.yaml"
...
def parse_cli_args() -> Tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(
        description="Finetune Gemma3-VL with YAML and CLI overrides",
    ...
    parser.add_argument(
        ...
        help="Path to the YAML OmegaConf override file. Default: conf/gemma3_vl_pretrain_override_example.yaml",
    )
    parser.add_argument(
        "--data-path",
        type=str,
    ...
    parser.add_argument(
        "--recipe",
        type=str,
        default="gemma3_vl_4b_finetune_config",
        help=(
            "Name of the recipe function in megatron.bridge.recipes.gemma3_vl.gemma3_vl to use, "
            "e.g., gemma3_vl_4b_finetune_config, gemma3_vl_12b_finetune_config, gemma3_vl_27b_finetune_config."
        ),
    )
    parser.add_argument(
...
def main() -> None:
    ...
    logger.info("Megatron-Bridge Gemma3-VL Finetuning Script with YAML & CLI Overrides")
    logger.info("-----------------------------------------------------------------------")

    # Resolve the recipe function from the provided name
    recipe_name = getattr(args, "recipe", "gemma3_vl_4b_finetune_config")
    available_recipes = [name for name in dir(gemma3_vl_recipes) if name.endswith("_finetune_config")]
    if not hasattr(gemma3_vl_recipes, recipe_name):
        logger.error(
        ...
        sys.exit(2)
    finetune_config = getattr(gemma3_vl_recipes, recipe_name)
