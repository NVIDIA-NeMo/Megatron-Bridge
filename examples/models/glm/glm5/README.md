...
[slurm_inference.sh](slurm_inference.sh) loads the HF checkpoint, converts to Megatron in-memory, and runs greedy text generation with `TP=1, PP=2, EP=32` across 64 GPUs. TP remains disabled because AbsorbedMLA requires sequence parallelism when `TP > 1`. The launcher passes `--no-kv-cache` to use legacy token-by-token generation, which recomputes the full accumulated prefix on every step. Cached MCore inference is not yet supported for AbsorbedMLA.
