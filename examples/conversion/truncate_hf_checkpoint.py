...
"""Create a shallow Hugging Face checkpoint by retaining its first transformer layers.

Unlike ``create_hf_toy_model.py``, this tool preserves weights from a pretrained
checkpoint. It rewrites safetensors files as a byte stream, so even large or
sharded checkpoints do not need to be loaded into host memory.
"""
...
def _prepare_output(source_dir: Path, output_dir: Path, *, overwrite: bool) -> Path:
    ...
    if output_dir == source_dir or source_dir in output_dir.parents:
        raise ValueError("output_dir must not be the source directory or one of its subdirectories")
