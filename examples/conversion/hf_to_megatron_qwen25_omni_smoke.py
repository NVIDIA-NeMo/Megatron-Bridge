...
"""Run a single-rank Qwen2.5-Omni thinker smoke forward on one real image+audio sample."""
...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for the Qwen2.5-Omni smoke forward example."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--hf-model-path",
        type=Path,
        required=True,
        help="Path to the local Hugging Face Qwen2.5-Omni thinker checkpoint.",
    )
    parser.add_argument(
        "--sample-parquet",
        type=Path,
