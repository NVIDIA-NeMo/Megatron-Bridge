...
grep -q 'mv baseline/Megatron-LM 3rdparty/Megatron-LM' "$dockerfile"
...
test "$(grep -c 'case "$MCORE_REPO" in' "$workflow")" = 2
test "$(grep -c 'https://github.com/NVIDIA/Megatron-LM.git|https://github.com/svcnemo-autobot/Megatron-LM.git)' "$workflow")" = 2
