...
def main() -> None:
    ...
    parser.add_argument(
        "version",
        help="Version folder name (e.g. v0.2.0)",
    )
    parser.add_argument(
        "--docs-dir",
        default="docs",
    ...
    # Copy docs/training/images and docs/images to fern/assets/training/images (Megatron-Bridge)
    fern_training_images = fern_assets / "training" / "images"
    fern_training_images.mkdir(parents=True, exist_ok=True)

    docs_training_images = docs_dir / "training" / "images"
    if docs_training_images.exists():
        for img in docs_training_images.iterdir():
            if img.is_file():
                shutil.copy2(img, fern_training_images / img.name)
        print(f"Copied training images from {docs_training_images} to {fern_training_images}")
    ...
    if docs_images.exists():
        for img in docs_images.iterdir():
            if img.is_file():
                shutil.copy2(img, fern_training_images / img.name)
        print(f"Copied docs/images to {fern_training_images}")

    # Copy repo root images (e.g. Repo-Mbridge.png from README) to fern/assets
    repo_root = docs_dir.parent
    for img_name in ["Repo-Mbridge.png"]:
        src = repo_root / img_name
        if src.exists():
            shutil.copy2(src, fern_assets / img_name)
            print(f"Copied {img_name} to {fern_assets}")
