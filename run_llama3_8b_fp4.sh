#!/bin/bash
# Usage:
#   Normal run: ./run_llama3_8b_fp4.sh
#   Deterministic mode: DETERMINISTIC=true ./run_llama3_8b_fp4.sh
#   Deterministic with Flash Attention: DETERMINISTIC=true BACKEND=flash ./run_llama3_8b_fp4.sh
set -euo pipefail
source ../../secrets.sh
...
if [ "$GPU" = "h100" ]; then
    ...
    echo "fp4 not supported on h100, invalid GPU: $GPU"
    exit 1
...
python scripts/performance/setup_experiment.py \
    ...
    -wdj "llama3-8b-fp4-nemo-25.11-${EXP_NAME}" \
    --task pretrain \
    -mb 2 \
