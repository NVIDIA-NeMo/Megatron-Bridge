...
parity. `--task full-prefix-generation` selects the compatibility generation
entry point. Pair it with `--legacy-full-prefix` to recompute the accumulated
prefix for every decoding step for models such as GLM-5 whose AbsorbedMLA
attention does not yet support cached inference.
