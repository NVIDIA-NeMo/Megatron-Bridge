...
Megatron-LM checkpoints with saved arguments in `common.pt` can be exported
without `run_config.yaml` when Bridge supports the model architecture. See
[exporting an existing checkpoint](../../docs/megatron-lm-to-megatron-bridge.md#export-an-existing-megatron-lm-checkpoint)
for an example. Only load checkpoints you trust.
