...
def qwen3_30b_a3b_gb200_8gpus_bf16_config(**kwargs) -> ConfigContainer:
    ...
    cfg.train.micro_batch_size = 4 if mbs is None else mbs
...
def qwen3_30b_a3b_b200_8gpus_bf16_config(**kwargs) -> ConfigContainer:
...
def qwen3_30b_a3b_h100_16gpus_bf16_config(**kwargs) -> ConfigContainer:
    ...
    cfg.model.tensor_model_parallel_size = 1 if tp is None else tp
    cfg.model.pipeline_model_parallel_size = 2 if pp is None else pp
    cfg.model.context_parallel_size = 1 if cp is None else cp
    cfg.model.virtual_pipeline_model_parallel_size = 12 if vp is None else vp
    cfg.model.expert_model_parallel_size = 8 if ep is None else ep
    cfg.model.expert_tensor_parallel_size = 1 if etp is None else etp
    cfg.model.sequence_parallel = bool(cfg.model.tensor_model_parallel_size > 1)
...
def qwen3_30b_a3b_h100_16gpus_fp8_config(**kwargs) -> ConfigContainer:
