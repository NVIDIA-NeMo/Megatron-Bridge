#!/usr/bin/env bash
# End-to-end MXFP8 det-ON-vs-det-OFF comparison for Nemotron 3 Ultra.
#
# Identical to launch_nemotron_3_ultra_nsys_compare.sh EXCEPT the compute dtype is
# MXFP8 (``-c fp8_mx`` instead of ``-c bf16``). ``-c fp8_mx`` routes through
# get_precision_config("fp8_mx") -> bf16_with_mxfp8_mixed() (fp8="e4m3",
# fp8_recipe="mxfp8", fp8_param_gather=True, reuse_grad_buf_for_mxfp8_param_ag=True).
#
# The two compared arms are the same determinism A/B as the bf16 compare script,
# now under MXFP8:
#   * "det"    arm = MXFP8 + deterministic_mode=true  + the 4 det env vars   (det ON)
#   * "nondet" arm = MXFP8 + deterministic_mode=false                        (det OFF)
# So the perf leaderboard is a det-ON vs det-OFF comparison *at MXFP8*, and the
# bit-wise check asks whether MXFP8 + deterministic_mode even reproduces across
# allocations. NOTE: FP8 quantization is NOT bit-exact validated — do not assume
# the det arm is bit-reproducible here; that is exactly what the check measures.
#
# Submits FOUR jobs and produces both reports:
#   1. det + nsys      (perf-comparison side A — det ON,  MXFP8)
#   2. non-det + nsys  (perf-comparison side B — det OFF, MXFP8)
...
#   nsys-nondet.csv   -- NVTX nvtx_sum CSV for det-OFF run (rank 0)
#   leaderboard.txt   -- side-by-side report (top-20 by |delta|)
#   bitwise_check.txt -- det-ON reproducibility under MXFP8
#
...
submit_run() {
    ...
    case "$MODE" in
        det)          WDJ="nemotron-3-ultra-mxfp8-det-nsys${NSYS_START}-${NSYS_STOP}-${TS}" ;;
        nondet)       WDJ="nemotron-3-ultra-mxfp8-nondet-nsys${NSYS_START}-${NSYS_STOP}-${TS}" ;;
        det-bitwise)  WDJ="nemotron-3-ultra-mxfp8-det-bitwise-check-${TS}" ; ENABLE_NSYS=false ;;
        det-bitwise2) WDJ="nemotron-3-ultra-mxfp8-det-bitwise-check2-${TS}" ; ENABLE_NSYS=false ;;
    esac

    # Determinism delta (the det-ON vs det-OFF axis, held constant otherwise):
    #   det / det-bitwise / det-bitwise2: 4 det env vars + deterministic_mode=true  + cross_entropy_loss_fusion=false
...
# Four jobs total:
#   det:          det ON  + nsys           (perf-comparison side A, MXFP8)
#   nondet:       det OFF + nsys           (perf-comparison side B, MXFP8)
#   det-bitwise:  det ON  + NO nsys        (reproducibility check #1 —
...
# Side-by-side leaderboard for the perf comparison (det-ON+nsys vs det-OFF+nsys, MXFP8).
"$PYTHON" "$REPO_ROOT/scripts/performance/perf_leaderboard/print_nsys_leaderboard.py" "$OUT_DIR" \
    | tee "$OUT_DIR/leaderboard.txt"

# --- Bit-wise reproducibility check (MXFP8 + deterministic_mode) ---
# nsys-on vs nsys-off (det vs det-bitwise) — measures the nsys instrumentation effect
# on the MXFP8 deterministic recipe; det-bitwise vs det-bitwise2 — two independent
# no-nsys allocations. FP8 is not bit-exact validated, so a mismatch is informative,
# not necessarily a bug.
echo ""
echo "=== Bit-wise reproducibility check (MXFP8 + deterministic_mode) ==="
...
diff_table() {
    ...
    [ "$match_all" -eq 1 ] && echo "  → all sampled iters match exactly" \
                            || echo "  → at least one sampled iter disagrees (expected under FP8 — see note)"
}
{
    # nsys-on vs nsys-off: measures the nsys instrumentation effect on the MXFP8 det recipe.
    diff_table "det+nsys"    "det+no-nsys"    "$DET_LOG"  "$BIT_LOG"
    # two independent no-nsys det allocations vs each other: the MXFP8 determinism-trust check.
