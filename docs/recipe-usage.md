...
For dataset fields, prefer `seq_length` in Bridge examples. LLM pretraining uses `GPTDatasetConfig` with `data_path`, `blend`, or `blend_per_split`; SFT and PEFT use `dataset_root` for a local JSONL split directory or `per_split_data_args_path` for MLM-style JSONL blends. Do not use `data_path` for SFT/PEFT JSONL sources.
