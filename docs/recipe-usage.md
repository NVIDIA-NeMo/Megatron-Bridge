...
Flat performance recipes keep feature-dependent environment values static and visible instead of recomputing them after command-line overrides. When overriding a feature that affects the process environment—such as CUDA graph mode, NCCL user buffers, dispatcher or parallelism settings—override the corresponding `env_vars` entries explicitly as well. For example, HybridEP topology changes must update `NUM_OF_HYBRID_EP_RANKS_PER_NVLINK_DOMAIN`, `NVLINK_DOMAIN_SIZE`, and `USE_MNNVL` together.
