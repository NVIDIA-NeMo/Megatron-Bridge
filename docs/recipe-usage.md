...
| VLM SFT or PEFT | `DirectHFSFTDatasetConfig` with hosted HF or local JSON/JSONL, or Energon | `finetune()` with a VLM step function | Use the model-specific checkpoint guidance in the recipe or model docs |
