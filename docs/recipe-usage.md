...
Flat performance recipes compose their process settings after the recipe is resolved. Common rules are feature-derived: dispatcher backend selects the LayerNorm SM margin, GPU and parallelism select `CUDA_DEVICE_MAX_CONNECTIONS`, HybridEP selects the NVLink topology, CUDA graph mode selects allocator/record-stream behavior, and precision selects settings such as `NVTE_USE_FAST_MATH`. A small number of measured model exceptions remain family-specific. This keeps every resolved recipe self-describing without copying the same environment dictionary into every config function.
