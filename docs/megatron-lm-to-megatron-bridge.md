...
## Export an existing Megatron-LM checkpoint

Megatron-LM checkpoints save model arguments in `common.pt`, not Bridge's
`run_config.yaml`. The export launcher reads those arguments directly for
architectures supported by Megatron Bridge.

```bash
./scripts/conversion/convert.sh export \
  --executor local --device cpu \
  --hf-model /workspace/hf-snapshots/model \
  --megatron-path /workspace/checkpoints/model/iter_0001000 \
  --hf-path /workspace/exports/model-hf
```

The HF reference must match the checkpoint's architecture. For distributed
export, see the [conversion launcher](../scripts/conversion/README.md).
