...
The public Slurm launcher is `scripts/training/train.sh`, backed by `setup_experiment.py`, which executes `run_recipe.py` directly inside the training container. Select exactly one model stem with `--model` or one complete library recipe function with `--recipe`, and always pass `--mode pretrain`, `sft`, `lora`, or `dora`. Public dataset names such as `dclm`, `openmathinstruct2`, `local-jsonl`, `preloaded-vlm`, and `mock` resolve to the corresponding dataset config; internal dataset config names are not launcher options. The default `--step-func` is `llm_step`; pass another registered step explicitly for non-LLM recipes. The launcher reads the container from `--container-image` or `CONTAINER_IMAGE` and forwards only explicit `--env NAME` or `--env NAME=VALUE` values and `--mount HOST` or `--mount HOST:CONTAINER` paths. Inherited `--env NAME` values are not serialized into the generated sbatch script, so use that form for secrets. `pretrain` mode runs `pretrain()`; SFT, LoRA, and DoRA run `finetune()`.
