...
<Note>
This documentation covers **Megatron-format checkpoints** used during training. For converting between 🤗 Hugging Face and Megatron formats, see the [bridge guide](/get-started/quickstart/hugging-face-conversion).
</Note>
...
**Important**: When using Megatron FSDP (`use_megatron_fsdp=True`), you must set `ckpt_format="fsdp_dtensor"`. Other formats are not compatible with FSDP's sharded parameter layout. See [megatron fsdp](/training/megatron-fsdp) for complete FSDP configuration details.
...
- [megatron fsdp](/training/megatron-fsdp) - Megatron FSDP configuration and `fsdp_dtensor` format requirements
- [parallelisms](/training/parallelisms) - Understanding data and model parallelism strategies
- [config container overview](/training/config-container-overview) - Complete configuration reference
