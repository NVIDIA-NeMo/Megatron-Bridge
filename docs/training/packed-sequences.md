...
the deprecated NumPy packed format is not supported. A packed blend cannot use
`pad_cu_seqlens` because CUDA-graph padding requires source-specific packing
metadata. A standalone blend must disable validation and test; set
`dataset_root` as well when those splits should be built from local files.
