...
## Padding masks for unpacked text MoE fine-tuning

For unpacked `GPTSFTDatasetConfig` data, opt into a token padding mask when
training MoE models with right padding:
...
This applies to both prompt-completion and chat datasets. The mask excludes
right padding and autogenerated sampler rows from router auxiliary loss and
expert statistics. Real prompt, tool, and EOS tokens remain valid even when
they are excluded from the language-model loss. The training step carries the
mask through context and sequence parallelism and every pipeline stage without
changing the unpacked attention layout. The option defaults to `False` to
preserve existing behavior. Packing uses its existing mask generation.
Router-scoped Transformer Engine CUDA graphs cannot consume these changing
