...
Performance scope: pretrain_performance.GB200 uses the tuned canonical language-and-projector MockVLM recipe and records 819.190 corrected model TFLOP/s/GPU on 16 GB200 GPUs; timing from all functional items remains sanity evidence. All workloads in this card target GB200 hardware. Conversion, deterministic inference, pretraining, SFT, long-context SFT, PEFT, and post-SFT export are verified. Full-language checkpoint resume remains unverified because four intermediate losses exceeded the comparison bound. No dense-27B Megatron FSDP recipe is available.
...
| Field | Value |
| --- | --- |
| Card | [`examples/model_verification_cards/qwen3.8-27b/card.yaml`](https://github.com/NVIDIA-NeMo/Megatron-Bridge/blob/main/examples/model_verification_cards/qwen3.8-27b/card.yaml) |
| Hugging Face model | [`Qwen/Qwen3.8-27B`](https://huggingface.co/Qwen/Qwen3.8-27B) |
| Hugging Face revision | `1d4bf0f2ff6012fd82039f2fa52739d0dd7c60c0` |
| Architecture | `Qwen3_5ForConditionalGeneration` |
| Minimum Transformers | `5.8.0` |
| Base container | `nvcr.io/nvidia/nemo:26.08.rc5` |
| Card-level Bridge commit | `d790e32a88654968021291d68c510f41d64df461` |
| Complementary model documentation | [Open model guide](../../models/README.md) |
| Aggregate state | **partially verified** |

Concrete status counts: ✅ verified: **12**, ⚪ unverified: **1**, ❌ unsupported: **0**, ➖ not applicable: **0**.
...
<a id="qwen3-8-27b-hf-to-megatron-cpu"></a>
### hf_to_megatron_cpu
...
| Effective Bridge commit | `d790e32a88654968021291d68c510f41d64df461` |

#### Explicit command topology

| Command | Explicit topology |
| --- | --- |
| 1 | `nodes=1, gpus_per_node=1, total_gpus=1` |
...
<a id="qwen3-8-27b-hf-to-megatron-gpu"></a>
### hf_to_megatron_gpu
...
The pinned-revision four-GB200 import completed in 2m01s at TP4/PP1 and created an 11-file iter_0000000 totaling 55,592,033,255 bytes, including four distributed checkpoint shards. All 956 conversion mappings completed, and subsequent export and two independent inference runs reloaded it.

<a id="qwen3-8-27b-megatron-to-hf-cpu"></a>
### megatron_to_hf_cpu
...
<a id="qwen3-8-27b-megatron-to-hf-gpu"></a>
### megatron_to_hf_gpu
...
| Precision | `bf16` |
| Last verified | `2026-08-14` |
| Effective Bridge commit | `ab905ac72ccc9e45a081153655eda9ee469fdcff` |
...
| Effective Bridge commit | `2e945d0734f2f0fb764320dda9c90139d02b792d` |

#### Explicit command topology

| Command | Explicit topology |
| --- | --- |
| 1 | `nodes=4, gpus_per_node=4, recipe=qwen35_vl_27b_pretrain_16gpu_h100_bf16_mock_config, total_gpus=16` |
...
| Command | Explicit topology |
| --- | --- |
| 1 | `nodes=4, gpus_per_node=4, recipe=qwen35_vl_27b_sft_16gpu_h100_bf16_config, total_gpus=16` |
...
| --- | --- |
| 1 | `nodes=1, gpus_per_node=1, total_gpus=1` |
| 2 | `nodes=1, gpus_per_node=1, total_gpus=1` |
...
| Command | Explicit topology |
| --- | --- |
| 1 | `nodes=4, gpus_per_node=4, pipeline_parallel=2, context_parallel=2, sequence_length=8192, recipe=qwen35_vl_27b_sft_16gpu_h100_bf16_config, total_gpus=16` |
...
| Command | Explicit topology |
| --- | --- |
| 1 | `nodes=1, gpus_per_node=4, tensor_parallel=4, recipe=qwen35_vl_27b_peft_2gpu_h100_bf16_config, total_gpus=4` |
...
The four-GB200 run completed exactly 100 deterministic MedPix LoRA steps at TP4/PP1, MBS4/GBS32, and one-layer full-uniform activation recompute. Loss remained finite from 2.115661 to 1.558339 with zero skipped or NaN iterations. Steps 91-100 averaged 74,048.550 ms and 75.790 model TFLOP/s/GPU. Step-50 and step-100 adapter checkpoints each contain 8 files totaling 2,294,105,245 bytes, the tracker selects iteration 100, and the process exited successfully in 2h01m43s. The unchanged stock TP2 two-GPU recipe without activation recompute OOMed; a TP4 retry without recompute also OOMed near the approximately 184.3-GiB per-GPU device limit. The successful command records only the required topology and recompute config overrides, with no code change.

<a id="qwen3-8-27b-checkpoint-resume-gb200"></a>
...
| Command | Explicit topology |
| --- | --- |
| 1 | `nodes=4, gpus_per_node=4, recipe=qwen35_vl_27b_pretrain_16gpu_gb200_bf16_mock_config, total_gpus=16` |
