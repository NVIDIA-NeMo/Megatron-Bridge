...
Performance scope: pretrain_performance.GB200 and pretrain_performance.GB300 use tuned canonical performance recipes; timing and throughput metrics from functional training items remain sanity checks rather than optimized performance results. GPT-OSS 120B H100 GPU HF-to-Megatron import, GPU Megatron-to-HF export, manual HF/Megatron forward-pass correlation, deterministic Megatron inference, bounded H100 pretraining, H100 SFT, H100 PEFT, and H100 checkpoint resume, H100 SFT export/inference, and H100 long-context SFT are verified. GB200 and GB300 pretraining performance are verified. CPU conversion remains planned but not yet verified.
...
| Field | Value |
| --- | --- |
| Card | [`examples/model_verification_cards/gpt-oss-120b/card.yaml`](https://github.com/NVIDIA-NeMo/Megatron-Bridge/blob/main/examples/model_verification_cards/gpt-oss-120b/card.yaml) |
| Hugging Face model | [`openai/gpt-oss-120b`](https://huggingface.co/openai/gpt-oss-120b) |
| Hugging Face revision | `b5c939de8f754692c1647ca79fbf85e8c1e70f8a` |
| Architecture | `GptOssForCausalLM` |
| Minimum Transformers | `5.8.0` |
| Base container | `nvcr.io/nvidia/nemo:26.06` |
| Card-level Bridge commit | `a8038f6d99d8ade33e1b715ba79bc9185b073e59` |
| Complementary model documentation | [Open model guide](../../models/gpt_oss/gpt-oss.md) |
| Aggregate state | **partially verified** |

Concrete status counts: ✅ verified: **12**, ⚪ unverified: **2**, ❌ unsupported: **0**, ➖ not applicable: **0**.
...
<a id="gpt-oss-120b-hf-to-megatron-cpu"></a>
### hf_to_megatron_cpu
...
| Precision | `bf16` |
| Last verified | `—` |
| Effective Bridge commit | `a8038f6d99d8ade33e1b715ba79bc9185b073e59` |
...
<a id="gpt-oss-120b-hf-to-megatron-gpu"></a>
### hf_to_megatron_gpu
...
<a id="gpt-oss-120b-megatron-to-hf-cpu"></a>
### megatron_to_hf_cpu
...
```bash
./scripts/conversion/convert.sh export --executor slurm --device cpu --nodes 1 --hf-model unsloth/gpt-oss-120b-BF16 --megatron-path work/model-verification/gpt-oss-120b/cpu-megatron/iter_0000000 --hf-path work/model-verification/gpt-oss-120b/cpu-hf-export --torch-dtype bfloat16 --trust-remote-code
```

#### Expected result

Future verification must export the CPU-imported checkpoint to a public BF16 Hugging Face layout, strictly reload it as GptOssForCausalLM with attention_bias=true, and audit keys, shapes, dtypes, and values against the BF16 reference semantics.

<a id="gpt-oss-120b-megatron-to-hf-gpu"></a>
### megatron_to_hf_gpu
...
<a id="gpt-oss-120b-manual-forward-pass"></a>
### manual_forward_pass
...
Compares pinned OpenAI Hugging Face reference logits and the imported Megatron checkpoint on the prompt "The capital of France is the city of". Both predict next token 12650 (' Paris'); token match is true, cosine similarity is 0.999963, maximum absolute logit difference is 0.250000, and mean absolute logit difference is 0.022671.

<a id="gpt-oss-120b-inference"></a>
### inference
...
Completes one synchronous deterministic greedy generation through the
...
<a id="gpt-oss-120b-pretrain-h100"></a>
### pretrain · H100
...
| Command | Explicit topology |
| --- | --- |
| 1 | `nodes=8, gpus_per_node=8, sequence_length=4096, recipe=gpt_oss_120b_pretrain_config, total_gpus=64` |
...
| Command | Explicit topology |
| --- | --- |
| 1 | `nodes=4, gpus_per_node=8, tensor_parallel=2, sequence_length=2048, recipe=gpt_oss_120b_sft_32gpu_h100_bf16_config, total_gpus=32` |
...
<a id="gpt-oss-120b-sft-export-inference-h100"></a>
...
| Command | Explicit topology |
| --- | --- |
| 1 | `nodes=4, gpus_per_node=8, tensor_parallel=2, pipeline_parallel=4, expert_parallel=8, expert_tensor_parallel=1, total_gpus=32` |
...
The verified step-100 SFT checkpoint exports to a strict-reloadable BF16
...
<a id="gpt-oss-120b-sft-long-context-h100"></a>
...
| Command | Explicit topology |
| --- | --- |
| 1 | `nodes=6, gpus_per_node=8, sequence_length=32768, recipe=gpt_oss_120b_sft_48gpu_h100_bf16_32k_config, total_gpus=48` |
...
| Command | Explicit topology |
| --- | --- |
| 1 | `nodes=1, gpus_per_node=8, sequence_length=2048, recipe=gpt_oss_120b_peft_8gpu_h100_bf16_config, total_gpus=8` |
...
<a id="gpt-oss-120b-checkpoint-resume-h100"></a>
...
<a id="gpt-oss-120b-pretrain-performance-gb200"></a>
...
| Command | Explicit topology |
| --- | --- |
| 1 | `nodes=16, gpus_per_node=4, recipe=gpt_oss_120b_pretrain_64gpu_gb200_bf16_config, total_gpus=64` |
...
<a id="gpt-oss-120b-pretrain-performance-gb300"></a>
...
| Command | Explicit topology |
| --- | --- |
| 1 | `nodes=16, gpus_per_node=4, sequence_length=4096, recipe=gpt_oss_120b_pretrain_64gpu_gb300_fp8mx_config, total_gpus=64` |
